...
El administrador puede publicar y editar banners desde el módulo **Publicidad**. El formato del piloto es `1200 × 400 px`, JPG, PNG o WebP, con un máximo de 1 MB. Las imágenes permanecen en la parte superior de la pantalla del pasajero antes de solicitar y durante el viaje; no se muestran al conductor para no distraer su operación. La aplicación consulta las campañas vigentes cada cinco minutos y alterna varias piezas cada ocho segundos. Cada campaña comercial tiene inicio y finalización automática. Cuando no existen campañas vigentes se utiliza permanentemente el banner demostrativo `Tu publicidad aquí`, que no requiere fechas. Activar, desactivar o modificar una campaña no requiere recompilar la APK.
...
- La aplicación ya no inicia con coordenadas fijas de Atacames. El origen se
  ...
  tocando el mapa.
- El proveedor móvil se selecciona con `MAP_PROVIDER`: `google` es el valor
  predeterminado y `osm` queda como respaldo explícito para diagnósticos.
