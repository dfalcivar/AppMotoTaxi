import {ManagedTable,useConsoleState,MetricStrip} from './console-ui';
...
import { ScheduledArrivalSettings } from './scheduled-arrival-settings.js';
import {CommercialEconomicsPanel} from './commercial-economics-panel.js';
import { buildMembershipPlanRequest, type MembershipPlanDraft } from './membership-plan-form.js';
...
const date = (value:unknown) => value ? ecuDate(value) : "Sin fecha";
...
const membershipLabels: Record<string,string> = {
  PENDING:"Pendiente de activación",ACTIVE:"Activa",EXPIRING:"Próxima a vencer",GRACE_PERIOD:"En gracia",EXHAUSTED:"Viajes agotados",
...
const paymentMethodLabels: Record<string,string> = { CASH:"Efectivo",DEUNA:"Transferencia",BANK_TRANSFER:"Transferencia bancaria",COURTESY:"Cortesía" };
...
function CollectorPortal({token}:Pick<Props,"token">) {
  ...
  return <div className="membership-admin collector-portal">
    ...
    <section className="card"><h2>Buscar orden</h2><p className="note">Escanea el QR o busca por código de orden, placa o correo del conductor.</p><div className="membership-toolbar"><label>QR, código, placa o correo<input value={lookup} onChange={event=>setLookup(event.target.value)} onKeyDown={event=>{if(event.key==="Enter"&&searchValue().length>=3)void findOrder();}} placeholder="Ej. ABC123, correo@dominio.com o código"/></label><label>Punto<select value={pointId} onChange={event=>setPointId(event.target.value)}>{profile.points?.map((point:any)=><option key={point.id} value={point.id}>{point.name}</option>)}</select></label><button className="primary" disabled={busy||searchValue().length<3} onClick={()=>void findOrder()}>Buscar</button></div>{orderMatches.length>1&&<div className="collector-order-results"><h3>Selecciona la orden correcta</h3>{orderMatches.map(match=><button key={match.id} type="button" onClick={()=>setOrder(match)}><span><strong>{match.driver}</strong><small>{match.email} · placa {match.vehicle??"sin registrar"}</small></span><span>{match.shortCode}<small>{match.plan?.planType==="TRIP_PACK"?"Plan por viajes":"Plan por período"} · {money(match.amount,match.currency)}</small></span></button>)}</div>}{order&&<div className="collector-order"><h3>{order.driver}</h3><p>{order.email} · placa <strong>{order.vehicle??"sin registrar"}</strong></p><p><strong>{order.plan?.name}</strong><br/><span className="membership-badge active">{order.plan?.planType==="TRIP_PACK"?`Plan por viajes · ${order.plan?.purchasedTrips??order.plan?.includedTrips} viajes`:"Plan por período"}</span></p><div className="collector-order-tax"><span>Subtotal</span><strong>{money(order.subtotalAmount,order.currency)}</strong><span>IVA ({Number(order.vatRatePercent??0).toLocaleString("es-EC")}%)</span><strong>{money(order.vatAmount,order.currency)}</strong><span>Total a cobrar</span><strong>{money(order.amount,order.currency)}</strong></div><p>Código {order.shortCode} · vence {date(order.expiresAt)}</p><FiscalProfileCard key={order.id} endpoint={`/v1/admin/fiscal/context/membership/${order.id}`} token={token} onReady={setFiscalReady}/><label>Método<select value={method} onChange={event=>setMethod(event.target.value)}><option value="CASH">Efectivo</option><option value="DEUNA">Transferencia</option><option value="BANK_TRANSFER">Transferencia verificada en punto</option></select></label><button className="primary" disabled={busy||!fiscalReady||order.status!=="PENDING"} onClick={()=>void confirm()}>{order.status==="PENDING"?"Confirmar pago":"Orden no disponible para cobro"}</button></div>}</section>
    <section className="card"><div className="membership-toolbar"><div><h2>Recaudación pendiente de cierre</h2><p>{selectedPayments.length} pago(s) · {money(selectedTotal)}</p></div>{pendingDates.length>0&&<label>Jornada pendiente<select value={closingDate} onChange={event=>setClosingDate(event.target.value)}>{pendingDates.map(value=><option key={value} value={value}>{businessDate(value)}</option>)}</select></label>}<button className="secondary" disabled={busy||!pointId||!closingDate||!selectedPayments.length} onClick={()=>void closeCash()}>{selectedPayments.length?"Cerrar jornada":"Sin valores pendientes"}</button></div>{selectedPayments.length?<div className="table-wrap"><ManagedTable><thead><tr><th>Hora</th><th>Conductor</th><th>Método</th><th>Monto</th><th>Estado</th></tr></thead><tbody>{selectedPayments.map(item=><tr key={item.id}><td>{date(item.confirmedAt)}</td><td>{item.driver}</td><td>{labelFor(paymentMethodLabels,item.method)}</td><td>{money(item.amount,item.currency)}</td><td>{labelFor(operationalStatusLabels,item.settlementStatus)}</td></tr>)}</tbody></ManagedTable></div>:<p className="empty">No hay nada que cerrar. La jornada ya fue cerrada o todavía no existen cobros pendientes.</p>}</section>
    <section className="card"><h2>Últimos cierres</h2><div className="table-wrap"><ManagedTable><thead><tr><th>Período</th><th>Punto</th><th>Total</th><th>Estado</th></tr></thead><tbody>{closures.map(item=><tr key={item.id}><td>{date(item.periodEnd)}</td><td>{item.point}</td><td>{money(item.netAmount)}</td><td>{labelFor(operationalStatusLabels,item.status)}</td></tr>)}</tbody></ManagedTable></div>{!closures.length&&<p className="empty">Aún no existen cierres con recaudación.</p>}</section>
    {dialog.modal}
  </div>;
}
...
function MembershipManagement({token,permissions}:Props) {
  ...
  const [filters,setFilters]=useConsoleState("membership-filters",{status:routeQuery().get("status")??"",search:routeQuery().get("q")??"",insight:routeQuery().get("insight")??""});
  useEffect(()=>{const params=routeQuery();if(params.has("status")||params.has("q")||params.has("insight"))setFilters(v=>({...v,status:params.get("status")??"",search:params.get("q")??"",insight:params.get("insight")??""}));},[]);
  const [tab,setTab]=useConsoleState<"memberships"|"plans"|"trip-plans"|"payments"|"settings"|"import"|"collection"|"grace">("tab",()=>{const requested=routeQuery().get("sub");const guards:Record<string,string>={memberships:"memberships:view",plans:"membership_plans:manage","trip-plans":"membership_plans:manage",payments:"payments:transfer_review",settings:"settings:view",import:"membership_import:manage",collection:"collection_points:manage",grace:"membership_grace:manage"};return requested&&guards[requested]&&can(guards[requested]!)?requested as "memberships":"memberships";});
  ...
  const plansOfType=(planType:"PERIODIC"|"TRIP_PACK",current:boolean)=>plans.filter(plan=>(plan.planType??"PERIODIC")===planType&&(plan.current===true)===current);

  const [page,setPage]=useState(1),[pageSize,setPageSize]=useConsoleState("membership-page-size",25);
  function showInsight(insight:string){setTab("memberships");setPage(1);setFilters({status:"",search:"",insight});}
  async function load() {
    setError("");
    try {
      const query=new URLSearchParams({page:String(page),limit:String(pageSize)});if(filters.insight)query.set("insight",filters.insight);if(filters.status)query.set("status",filters.status);if(filters.search)query.set("search",filters.search);
  ...
  useEffect(()=>{void load();},[token,filters.status,filters.insight,page,pageSize]);
  ...
  async function activateCourtesyForDriver(){
    const enabledPlans=plans.filter(plan=>plan.current===true);
    if(!enabledPlans.length){setError("No existen planes activos para aplicar la cortesía.");return;}
    setBusy(true);setError("");
    let drivers:any[]=[];
    try{drivers=await apiFetch<any[]>("/v1/admin/membership-courtesy-candidates",token);}
    catch(reason){setBusy(false);setError(reason instanceof Error?reason.message:"No se pudo cargar la lista de conductores.");return;}
    setBusy(false);
    if(!drivers.length){setError("No existen conductores aprobados disponibles para asignar una cortesía.");return;}
    const values=await dialog.open({title:"Asignar membresía de cortesía",description:"Selecciona un conductor y el plan. Puede tener Pago por uso o una membresía vigente; la activación quedará auditada y no registrará ingresos.",confirmLabel:"Activar cortesía",size:"wide",fields:[{name:"driverId",label:"Conductor",type:"select",initialValue:String(drivers[0].id),required:true,options:drivers.map(driver=>({value:String(driver.id),label:`${driver.name} · ${driver.email} · ${driver.currentPlan?`${driver.currentPlan} (${membershipLabels[driver.membershipStatus]??driver.membershipStatus})`:"Pago por uso"}`}))},{name:"planId",label:"Plan de cortesía",type:"select",initialValue:String(enabledPlans[0].id),required:true,options:enabledPlans.map(plan=>({value:String(plan.id),label:`${plan.name} · ${money(plan.baseAmount,plan.currency)}`}))},{name:"reason",label:"Motivo obligatorio",type:"textarea",required:true,minLength:5}]});
    if(!values)return;
    await applyCourtesy(String(values.driverId),String(values.planId),String(values.reason));
  }
  ...
  async function courtesyRenew(driverId:string){
    ...
    if(!values)return;
    await applyCourtesy(driverId,String(values.planId),String(values.reason));
  }

  async function createTopUpOrder(item:any){
    setBusy(true);setError("");
    let config:any;try{config=await apiFetch<any>("/v1/admin/membership-topup-order/config",token);}
    catch(reason){setError(reason instanceof Error?reason.message:"No se pudo consultar el límite de recarga.");setBusy(false);return;}
    setBusy(false);
    const minimum=Number(config.minimumTopUp),maximum=Number(config.maximumTopUp);
    const values=await dialog.open({title:"Crear orden de recarga",description:`Conductor: ${item.driver} · ${item.email}. Se creará una orden pendiente; no se cobrará ni se acreditará saldo hasta confirmar un pago.`,confirmLabel:"Crear orden",fields:[{name:"amount",label:"Valor de recarga sin IVA (USD)",type:"number",initialValue:minimum,required:true,min:minimum,max:maximum,step:.01}]});
    if(!values)return;
    const amount=Number(values.amount);if(!Number.isFinite(amount)||amount<minimum||amount>maximum){setError(`La recarga debe estar entre ${money(minimum)} y ${money(maximum)}.`);return;}
    setBusy(true);setError("");setSuccess("");
    try{const order=await apiFetch<any>("/v1/admin/membership-topup-order",token,{method:"POST",body:JSON.stringify({driverId:item.driverId,topUpAmount:amount.toFixed(2),idempotencyKey:crypto.randomUUID()})});
      setSuccess(`Orden pendiente ${order.shortCode} para ${item.driver}. Recarga ${money(order.baseAmount)}; total a cobrar ${money(order.totalAmount)}. No se ha confirmado ningún pago.`);await load();}
    catch(reason){setError(reason instanceof Error?reason.message:"No se pudo crear la orden de recarga.");}finally{setBusy(false);}
  }

  async function applyCourtesy(driverId:string,planId:string,reasonValue:string){
    const plan=plans.find(item=>String(item.id)===planId);if(!plan)return;const reason=reasonValue.trim();
    setBusy(true);setError("");
  ...
  function createPlan(planType:"PERIODIC"|"TRIP_PACK"){
    setError("");
    setPlanEditor({mode:"create",draft:{planType,name:"",code:"",periodUnit:planType==="PERIODIC"?"MONTH":"DAY",periodCount:1,baseAmount:0,includedTrips:planType==="TRIP_PACK"?10:0,maxRenewalAmount:0,extraTripSharePercent:planType==="PERIODIC"?Number(settings?.membershipExtraTripSharePercent??40):0,packValidityDays:0}});
  }
  function editPlan(plan:any){
    setError("");
    setPlanEditor({mode:"edit",source:plan,draft:{planType:plan.planType??"PERIODIC",name:String(plan.name??""),code:String(plan.code??""),...inferPlanPeriod(plan),baseAmount:Number(plan.baseAmount??0),includedTrips:Number(plan.includedTrips??0),maxRenewalAmount:Number(plan.maxRenewalAmount??0),extraTripSharePercent:Number(plan.extraTripSharePercent??settings?.membershipExtraTripSharePercent??40),packValidityDays:Number(plan.packValidityDays??0)}});
  }
  async function submitPlan(){
    if(!planEditor)return;
    const request=buildMembershipPlanRequest(planEditor.draft);
    if(!request.ok){setError(request.message);return;}
    const {code,values}=request;
  ...
  async function settleClosure(closure:any){
    const values=await dialog.open({title:"Conciliar cierre",description:`Confirma la entrega íntegra de ${money(closure.grossAmount)}. La comisión estimada de ${money(closure.commissionAmount)} se pagará por separado tras revisar la factura del comercio.`,confirmLabel:"Confirmar liquidación",fields:[{name:"reference",label:"Referencia",required:true,minLength:3}]});if(!values)return;const reference=String(values.reference).trim();
  ...
  async function configurePointCommission(point:any){
    const values=await dialog.open({title:`Comisión · ${point.name}`,description:"La tarifa se aplica a cada pago confirmado incluido en un cierre nuevo. Las jornadas anteriores conservan su cálculo. Costa-Go paga la factura del comercio por separado.",confirmLabel:"Guardar comisión",fields:[
      {name:"enabled",label:"Activar comisión para cierres nuevos",type:"checkbox",initialValue:Boolean(point.commissionEnabled)},
      {name:"perPayment",label:"Valor contratado por pago (USD)",type:"number",initialValue:Number(point.commissionPerPayment??0),min:0,max:1000,step:.01,required:true},
      {name:"priceBasis",label:"Cómo está pactado el valor",type:"select",initialValue:String(point.commissionPriceBasis??"BASE_PLUS_TAX"),options:[{value:"BASE_PLUS_TAX",label:"Base imponible; el IVA de la factura se suma"},{value:"TOTAL_INCLUDING_TAX",label:"Total de la factura, IVA incluido"}],help:"El IVA se registra según la factura real del comercio."}
    ]});if(!values)return;
    setBusy(true);setError("");try{await apiFetch(`/v1/admin/collection-points/${point.id}/commission`,token,{method:"PATCH",body:JSON.stringify({enabled:Boolean(values.enabled),perPayment:Number(values.perPayment),priceBasis:String(values.priceBasis)})});setSuccess("Comisión actualizada para cierres futuros.");await load();}catch(reason){setError(reason instanceof Error?reason.message:"No se pudo guardar la comisión.");}finally{setBusy(false);}
  }
  async function registerCommissionInvoice(closure:any){
    const basis=closure.commissionPriceBasis==="TOTAL_INCLUDING_TAX"?"total de la factura":"subtotal antes de IVA";
    const values=await dialog.open({title:`Factura del comercio · ${closure.point}`,description:`Comisión pactada: ${money(closure.commissionAmount)} por ${closure.commissionPaymentCount} pago(s). El ${basis} debe coincidir con ese importe. Verifica la factura en SRI antes de aprobarla.`,confirmLabel:"Registrar para revisión",fields:[
      {name:"supplierRuc",label:"RUC del comercio",required:true,minLength:13},
      {name:"invoiceNumber",label:"Número de factura (001-001-000000001)",required:true,minLength:17},
      {name:"accessKey",label:"Clave de acceso SRI (49 dígitos)",required:true,minLength:49},
      {name:"subtotal",label:"Subtotal USD",type:"number",initialValue:closure.commissionPriceBasis==="BASE_PLUS_TAX"?Number(closure.commissionAmount):0,min:0,step:.01,required:true},
      {name:"vatAmount",label:"IVA de la factura USD",type:"number",initialValue:0,min:0,step:.01,required:true},
      {name:"total",label:"Total USD",type:"number",initialValue:closure.commissionPriceBasis==="TOTAL_INCLUDING_TAX"?Number(closure.commissionAmount):0,min:.01,step:.01,required:true},
      {name:"pdf",label:"Factura del comercio en PDF",type:"file",required:true,help:"Máximo 3 MB. Finanzas la revisará junto con la clave de acceso SRI."}
    ]});if(!values)return;
    setBusy(true);setError("");try{const file=values.pdf as File;const pdfBase64=await new Promise<string>((resolve,reject)=>{const reader=new FileReader();reader.onload=()=>resolve(String(reader.result).split(",")[1]??"");reader.onerror=()=>reject(new Error("No se pudo leer el PDF."));reader.readAsDataURL(file);});await apiFetch(`/v1/admin/collection-closures/${closure.id}/commission-invoice`,token,{method:"POST",body:JSON.stringify({supplierRuc:String(values.supplierRuc).trim(),invoiceNumber:String(values.invoiceNumber).trim(),accessKey:String(values.accessKey).trim(),subtotal:Number(values.subtotal),vatAmount:Number(values.vatAmount),total:Number(values.total),pdfBase64})});setSuccess("Factura del comercio registrada para revisión.");await load();}catch(reason){setError(reason instanceof Error?reason.message:"No se pudo registrar la factura.");}finally{setBusy(false);}
  }
  async function viewCommissionInvoicePdf(closure:any){
    setBusy(true);setError("");try{const response=await fetch(apiUrl(`/v1/admin/collection-closures/${closure.id}/commission-invoice/pdf`),{headers:{authorization:`Bearer ${token}`},cache:"no-store"});if(!response.ok)throw new Error("No se pudo descargar la factura.");const url=URL.createObjectURL(await response.blob());const anchor=document.createElement("a");anchor.href=url;anchor.download=`comision-${closure.id}.pdf`;anchor.click();window.setTimeout(()=>URL.revokeObjectURL(url),60000);}catch(reason){setError(reason instanceof Error?reason.message:"No se pudo descargar la factura.");}finally{setBusy(false);}
  }
  async function reviewCommissionInvoice(closure:any){
    setBusy(true);let invoice:any;try{invoice=(await apiFetch<any>(`/v1/admin/collection-closures/${closure.id}/commission-invoice`,token)).invoice;}catch(reason){setError(reason instanceof Error?reason.message:"No se pudo consultar la factura.");}finally{setBusy(false);}if(!invoice)return;
    const values=await dialog.open({title:"Revisar factura del comercio",description:`${invoice.invoiceNumber} · RUC ${invoice.supplierRuc} · clave SRI ${invoice.accessKey} · subtotal ${money(invoice.subtotal)} + IVA ${money(invoice.vatAmount)} = ${money(invoice.total)}. Abre el PDF adjunto y confirma la autorización y los valores en SRI antes de aprobar.`,confirmLabel:"Guardar revisión",fields:[{name:"decision",label:"Decisión",type:"select",options:[{value:"",label:"Selecciona"},{value:"APPROVED",label:"Aprobar factura verificada"},{value:"REJECTED",label:"Rechazar factura"}],required:true},{name:"note",label:"Resultado de la validación / motivo",type:"textarea",required:true,minLength:8}]});if(!values)return;
    setBusy(true);setError("");try{await apiFetch(`/v1/admin/collection-closures/${closure.id}/commission-invoice/review`,token,{method:"POST",body:JSON.stringify({approved:values.decision==="APPROVED",note:String(values.note).trim()})});setSuccess("Revisión de factura registrada.");await load();}catch(reason){setError(reason instanceof Error?reason.message:"No se pudo revisar la factura.");}finally{setBusy(false);}
  }
  async function payCommissionInvoice(closure:any){
    const values=await dialog.open({title:"Registrar pago al comercio",description:`Paga ${money(closure.commissionInvoiceTotal)} de la factura aprobada. El cierre del punto debe estar conciliado por ${money(closure.grossAmount)}.`,confirmLabel:"Registrar pago",fields:[{name:"reference",label:"Referencia bancaria del pago",required:true,minLength:3}]});if(!values)return;
    setBusy(true);setError("");try{await apiFetch(`/v1/admin/collection-closures/${closure.id}/commission-invoice/pay`,token,{method:"POST",body:JSON.stringify({reference:String(values.reference).trim()})});setSuccess("Pago de comisión registrado.");await load();}catch(reason){setError(reason instanceof Error?reason.message:"No se pudo registrar el pago.");}finally{setBusy(false);}
  }
  ...
  return <div className="membership-admin">
    ...
    {summary&&<MetricStrip items={[{label:"Activas",value:summary.active,tone:"positive",onClick:()=>showInsight("active")},{label:"Vencen en 7 días",value:summary.expiring7Days,tone:"warning",onClick:()=>showInsight("expiring7Days")},{label:"En gracia",value:summary.grace,onClick:()=>showInsight("grace")},{label:"Vencidas",value:summary.expired,tone:"danger",onClick:()=>showInsight("expired")},{label:"Suspendidas",value:summary.suspended,tone:"danger",onClick:()=>showInsight("suspended")},{label:"Ingresos confirmados del mes",value:money(summary.confirmedIncomeMonth),hint:"Mes calendario actual"}]}/>}
    <div className="membership-tabs" role="tablist"><button className={tab==="memberships"?"active":""} onClick={()=>setTab("memberships")}>Membresías</button><button className={tab==="plans"?"active":""} onClick={()=>setTab("plans")}>Planes por período</button><button className={tab==="trip-plans"?"active":""} onClick={()=>setTab("trip-plans")}>Planes por viajes</button>{can("membership_grace:manage")&&<button className={tab==="grace"?"active":""} onClick={()=>setTab("grace")}>Campañas de gracia</button>}{can("payments:transfer_review")&&<button className={tab==="payments"?"active":""} onClick={()=>setTab("payments")}>Pagos pendientes</button>}{can("collection_points:manage")&&<button className={tab==="collection"?"active":""} onClick={()=>setTab("collection")}>Puntos y cierres</button>}{can("settings:view")&&<button className={tab==="settings"?"active":""} onClick={()=>setTab("settings")}>Parámetros</button>}{can("membership_import:manage")&&<button className={tab==="import"?"active":""} onClick={()=>setTab("import")}>Importar CSV</button>}</div>
    {tab==="memberships"&&<section className="card"><div className="membership-toolbar"><label>Estado<select value={filters.status} onChange={event=>{setPage(1);setFilters({...filters,status:event.target.value,insight:""});}}><option value="">Todos</option>{statusOptions.map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></label><label>Buscar<input value={filters.search} onChange={event=>setFilters({...filters,search:event.target.value})} placeholder="Nombre, correo o teléfono"/></label><button className="primary" onClick={()=>{if(page!==1)setPage(1);else void load();}}>Aplicar</button>{can("memberships:manage")&&can("payments:courtesy_grant")&&<button className="secondary" disabled={busy} onClick={()=>void activateCourtesyForDriver()}>+ Asignar cortesía</button>}</div>{filters.insight&&<p className="note">Filtro del indicador: {({active:"Activas",expiring7Days:"Vencen en 7 días",grace:"En gracia",expired:"Vencidas",suspended:"Suspendidas"} as Record<string,string>)[filters.insight]} <button className="link" onClick={()=>showInsight("")}>Quitar filtro</button></p>}<div className="table-wrap"><ManagedTable serverPaged><thead><tr><th>Conductor</th><th>Plan</th><th>Estado</th><th>Vigencia</th><th>Uso</th><th>Próxima renovación</th><th>Cobertura</th><th>Acciones</th></tr></thead><tbody>{memberships.items?.map((item:any)=><tr key={item.id}><td><strong>{item.driver}</strong><small>{item.email}<br/>{item.cooperative??"Individual"}</small></td><td>{labelFor(planLabels,item.plan)}<small>{item.planType==="TRIP_PACK"?"Plan por viajes":"Plan por período"}</small></td><td><span className={`membership-badge ${String(item.status).toLowerCase()}`}>{membershipLabels[item.status]??item.status}</span></td><td>{item.planType==="TRIP_PACK"&&!item.expiresAt?"Hasta agotar saldo":date(item.expiresAt)}{item.graceEndsAt&&<small>Gracia: {date(item.graceEndsAt)}</small>}</td><td>{item.planType==="TRIP_PACK"?<><strong>{item.remainingTrips} disponibles</strong><small>{item.completedTrips} consumidos · {item.includedTrips} acreditados</small></>:<>{item.completedTrips}/{item.includedTrips}<small>{item.extraTrips} adicionales</small></>}</td><td><strong>{item.planType==="TRIP_PACK"?"No aplica":money(item.estimatedRenewal,item.currency)}</strong></td><td>{item.payerType==="COOPERATIVE"?"Cooperativa":"Individual"}</td><td>{can("memberships:manage")?<div className="row-actions">{can("payments:courtesy_grant")&&<button className="link" disabled={busy} onClick={()=>void courtesyRenew(item.driverId)}>Activar cortesía</button>}<button className="link" disabled={busy} onClick={()=>void createTopUpOrder(item)}>Crear recarga</button><button className="link" disabled={busy} onClick={()=>void action(item.driverId,"REACTIVATE")}>Reactivar</button>{item.planType!=="TRIP_PACK"&&<><button className="link" disabled={busy} onClick={()=>void grantGrace(item.driverId)}>Dar gracia</button><button className="link" disabled={busy} onClick={()=>void adjustMembership(item.driverId)}>Ajustar</button></>}<button className="link danger" disabled={busy} onClick={()=>void action(item.driverId,"SUSPEND")}>Suspender</button></div>:"Solo lectura"}</td></tr>)}</tbody></ManagedTable></div>{!memberships.items?.length&&<p className="empty">No hay membresías para este filtro.</p>}<div className="cg-pagination"><label>Por página <select value={pageSize} onChange={e=>{setPageSize(Number(e.target.value));setPage(1);}}>{[10,25,50,100].map(n=><option key={n}>{n}</option>)}</select></label><span>{memberships.total??0} registros · página {page}</span><button disabled={page<=1} onClick={()=>setPage(p=>p-1)}>Anterior</button><button disabled={page*pageSize>=Number(memberships.total??0)} onClick={()=>setPage(p=>p+1)}>Siguiente</button></div></section>}
    {tab==="plans"&&renderPlans("PERIODIC")}
    {tab==="trip-plans"&&renderPlans("TRIP_PACK")}
    {tab==="trip-plans"&&<CommercialEconomicsPanel token={token} permissions={permissions} mode="packages"/>}
    {tab==="settings"&&<CommercialEconomicsPanel token={token} permissions={permissions} mode="settings"/>}
    {tab==="memberships"&&<CommercialEconomicsPanel token={token} permissions={permissions} mode="dashboard"/>}
    {tab==="collection"&&<>
      <section className="card"><h2>Comisiones por punto</h2><p className="note">Inicialmente desactivadas. La tarifa se aplica a cada pago confirmado de un cierre nuevo; el comercio factura el servicio a Costa-Go y se le paga aparte. El IVA proviene de esa factura.</p>
        <div className="table-wrap"><ManagedTable><thead><tr><th>Punto</th><th>Estado</th><th>Valor pactado</th><th>Base del precio</th><th>Acción</th></tr></thead><tbody>{collectionData.points?.map((point:any)=><tr key={point.id}><td>{point.name}</td><td>{point.commissionEnabled?"Activa":"Desactivada"}</td><td>{money(point.commissionPerPayment)} por pago</td><td>{point.commissionPriceBasis==="TOTAL_INCLUDING_TAX"?"Total con IVA":"Base + IVA"}</td><td><button className="link" disabled={busy} onClick={()=>void configurePointCommission(point)}>Configurar comisión</button></td></tr>)}</tbody></ManagedTable></div>
      </section>
      <section className="card"><h2>Facturas y pagos de comisiones</h2><p className="note">El punto entrega el total cobrado. La comisión estimada se paga solo después de registrar y verificar la factura del comercio.</p>
        <div className="table-wrap"><ManagedTable><thead><tr><th>Cierre / punto</th><th>Recaudación bruta</th><th>Comisión pactada</th><th>Factura del comercio</th><th>Acción</th></tr></thead><tbody>{closures.filter(item=>Number(item.commissionAmount)>0).map(item=><tr key={item.id}><td>{date(item.periodEnd)}<small>{item.point} · {item.commissionPaymentCount} pago(s)</small></td><td>{money(item.grossAmount)}<small>{item.status==="SETTLED"?"Conciliado":"Pendiente de conciliación"}</small></td><td>{money(item.commissionAmount)}<small>{item.commissionPriceBasis==="TOTAL_INCLUDING_TAX"?"Total con IVA":"Base + IVA"}</small></td><td>{item.commissionInvoiceStatus?labelFor({PENDING_REVIEW:"Pendiente de revisión",APPROVED:"Aprobada",REJECTED:"Rechazada",PAID:"Pagada"},item.commissionInvoiceStatus):"Sin registrar"}{item.commissionInvoiceTotal!=null&&<small>{money(item.commissionInvoiceTotal)}</small>}</td><td><div className="row-actions">{item.commissionInvoiceStatus&&<button className="link" disabled={busy} onClick={()=>void viewCommissionInvoicePdf(item)}>Descargar PDF</button>}{(!item.commissionInvoiceStatus||item.commissionInvoiceStatus==="REJECTED")&&<button className="link" disabled={busy} onClick={()=>void registerCommissionInvoice(item)}>Registrar factura</button>}{item.commissionInvoiceStatus==="PENDING_REVIEW"&&<button className="link" disabled={busy} onClick={()=>void reviewCommissionInvoice(item)}>Revisar factura</button>}{item.commissionInvoiceStatus==="APPROVED"&&<button className="link" disabled={busy||item.status!=="SETTLED"} onClick={()=>void payCommissionInvoice(item)}>Registrar pago</button>}</div></td></tr>)}</tbody></ManagedTable></div>{!closures.some(item=>Number(item.commissionAmount)>0)&&<p className="empty">Aún no hay comisiones generadas.</p>}
      </section>
    </>}
    {tab==="settings"&&can('settings:manage')&&<ScheduledArrivalSettings token={token}/>}
    {tab==="grace"&&<section className="card"><div className="membership-toolbar"><div><h2>Campañas de gracia</h2><p>Se crean en borrador, muestran el alcance y requieren activación explícita.</p></div><button className="primary" disabled={busy} onClick={()=>void createGraceCampaign()}>+ Nueva campaña</button></div><div className="table-wrap"><ManagedTable><thead><tr><th>Campaña</th><th>Alcance</th><th>Días</th><th>Vigencia</th><th>Estado</th><th>Acción</th></tr></thead><tbody>{gracePolicies.map(policy=><tr key={policy.id}><td><strong>{policy.name}</strong><small>{policy.reason}</small></td><td>{labelFor(scopeLabels,policy.scope)}</td><td>{policy.graceDays}</td><td>{date(policy.startsAt)}<br/>{date(policy.endsAt)}</td><td><span className={`membership-badge ${String(policy.status).toLowerCase()}`}>{labelFor(operationalStatusLabels,policy.status)}</span></td><td>{policy.status!=="FINISHED"?<button className="link" disabled={busy} onClick={()=>void toggleGracePolicy(policy)}>{policy.status==="ACTIVE"?"Pausar":"Activar"}</button>:"Finalizada"}</td></tr>)}</tbody></ManagedTable></div>{!gracePolicies.length&&<p className="empty">No existen campañas configuradas.</p>}</section>}
    {tab==="payments"&&<section className="card"><h2>Transferencias por verificar</h2><p className="muted">Contrasta el monto, la modalidad y la referencia con el movimiento real de la cuenta bancaria. Estas transferencias no requieren cierre de caja ni conciliación posterior.</p><div className="table-wrap"><ManagedTable><thead><tr><th>Conductor</th><th>Plan solicitado</th><th>Referencia bancaria</th><th>Declarado</th><th>Esperado</th><th>Fecha</th><th>Acción</th></tr></thead><tbody>{pendingPayments.map(item=><tr key={item.id}><td>{item.driver}</td><td><strong>{item.plan?.name}</strong><small>{item.plan?.planType==="TRIP_PACK"?`Plan por viajes · ${item.plan?.purchasedTrips??item.plan?.includedTrips} viajes`:"Plan por período"}</small></td><td><strong>{item.reference}</strong></td><td>{money(item.declaredAmount,item.currency)}</td><td>{money(item.expectedAmount,item.currency)}</td><td>{date(item.createdAt)}</td><td><div className="row-actions"><button className="link" onClick={()=>setFiscalOrder(item.orderId)}>Datos fiscales</button><button className="link" disabled={busy} onClick={()=>void openPaymentProof(item.id)}>Ver comprobante</button><button className="link" disabled={busy} onClick={()=>void reviewPayment(item,true)}>Verificar y aprobar</button><button className="link danger" disabled={busy} onClick={()=>void reviewPayment(item,false)}>Rechazar</button></div></td></tr>)}</tbody></ManagedTable></div>{!pendingPayments.length&&<p className="empty">No existen comprobantes pendientes de verificación bancaria.</p>}</section>}
    {tab==="collection"&&<><section className="card"><div className="membership-toolbar"><div><h2>Puntos de recaudación</h2><p>Directorio visible para conductores, ubicación, horarios, métodos y personal autorizado.</p></div><button className="primary" disabled={busy} onClick={()=>void createCollectionPoint()}>+ Nuevo punto</button></div><div className="table-wrap"><ManagedTable><thead><tr><th>Punto</th><th>Ubicación y contacto</th><th>Estado</th><th>Métodos</th><th>Recaudador</th><th>Acciones</th></tr></thead><tbody>{collectionData.points?.map((point:any)=><tr key={point.id}><td><strong>{point.name}</strong><small>{point.code}<br/>{point.serviceArea??"Sin zona"}</small></td><td>{point.address??"Sin dirección"}<small>{point.reference??"Sin referencia"}<br/>{point.phone??"Sin teléfono"}{point.latitude!=null?<><br/>{Number(point.latitude).toFixed(5)}, {Number(point.longitude).toFixed(5)}</>:null}</small></td><td><span className={`membership-badge ${String(point.status).toLowerCase()}`}>{labelFor(operationalStatusLabels,point.status)}</span></td><td>{[point.cashEnabled&&"Efectivo",point.deunaEnabled&&"Transferencia",point.bankTransferEnabled&&"Transferencia"].filter(Boolean).join(", ")||"Sin métodos"}</td><td>{point.collectors?.length?point.collectors.map((collector:any)=><span key={collector.id}>{collector.name}<br/></span>):"Sin asignar"}</td><td><div className="row-actions"><button className="link" disabled={busy} onClick={()=>void editCollectionPoint(point)}>Configurar</button></div></td></tr>)}</tbody></ManagedTable></div></section><section className="card"><h2>Cierres pendientes de conciliación</h2><div className="table-wrap"><ManagedTable><thead><tr><th>Fecha</th><th>Punto</th><th>Recaudador</th><th>Total</th><th>Estado</th><th>Acción</th></tr></thead><tbody>{closures.map(item=><tr key={item.id}><td>{date(item.periodEnd)}</td><td>{item.point}</td><td>{item.collector}</td><td>{money(item.netAmount)}</td><td>{labelFor(operationalStatusLabels,item.status)}</td><td>{item.status==="PENDING_SETTLEMENT"?<button className="link" disabled={busy} onClick={()=>void settleClosure(item)}>Conciliar</button>:"Verificado"}</td></tr>)}</tbody></ManagedTable></div>{!closures.length&&<p className="empty">No existen cierres registrados.</p>}</section></>}
    {tab==="settings"&&settings&&<section className="card"><h2>Parámetros operativos</h2><p className="note">La aplicación del bloqueo permanece independiente para poder probar cálculos y pagos sin bloquear conductores.</p><div className="settings-grid">{setting("driverMembershipsEnabled","Módulo de membresías","checkbox")}{setting("membershipEnforcementEnabled","Aplicar bloqueo por vigencia","checkbox")}{setting("membershipUsageBillingEnabled","Contabilizar viajes adicionales","checkbox")}{setting("membershipSuspensionSchedulerEnabled","Procesar suspensiones automáticamente","checkbox")}{setting("membershipExpiryNoticeDays","Avisar antes de vencer (días)")}{setting("membershipGraceDays","Gracia general (días)")}{setting("membershipGraceAllowsTrips","Permitir viajes durante gracia","checkbox")}{setting("membershipSuspensionLocalTime","Hora local de suspensión","text")}{setting("membershipTimezone","Zona horaria","text")}{setting("newDriverGraceEnabled","Gracia inicial al aprobar","checkbox")}{setting("newDriverGraceDurationHours","Gracia inicial (horas)")}{setting("newDriverGraceAllowsTrips","Permitir viajes en gracia inicial","checkbox")}{setting("membershipExtraTripSharePercent","Porcentaje sobre adicional del pasajero")}{setting("membershipQrDurationHours","Vigencia QR (horas)")}{navigationSetting("navigationPickupProvider","Navegación hacia el pasajero")}{navigationSetting("navigationDestinationProvider","Navegación hacia el destino")}{setting("textSearchMonthlyBudgetUsd","Presupuesto Text Search USD")}{setting("textSearchHardLimitEnabled","Detener Text Search en el límite","checkbox")}{setting("routesFreeCapReference","Franquicia mensual Routes")}{setting("routesPricePerThousandUsd","Precio Routes por 1.000")}{setting("geocodingFreeCapReference","Franquicia mensual Geocoding")}{setting("geocodingPricePerThousandUsd","Precio Geocoding por 1.000")}{setting("navigationPricePerThousandUsd","Precio Navigation por 1.000 destinos")}{setting("advertisingRotationSeconds","Rotación de publicidad (segundos)")}{setting("advertisingMaxActivePerZone","Campañas activas máximas por zona")}</div><button className="primary" disabled={busy||!can("settings:manage")} onClick={()=>void saveSettings()}>Guardar parámetros</button>{usage&&<div className="usage-summary"><h3>Consumo del período {usage.period}</h3><p>Text Search Pro: <strong>{usage.textSearch.used}</strong> · costo estimado {money(usage.textSearch.estimatedCost)}</p><p>Routes API: <strong>{usage.routes.used}</strong> · costo estimado {money(usage.routes.estimatedCost)}</p><p>Geocoding API: <strong>{usage.geocoding.used}</strong> · costo estimado {money(usage.geocoding.estimatedCost)}</p><p>Navigation SDK (opcional): <strong>{usage.navigation.used}</strong> · {usage.navigation.enabled?`habilitado · costo estimado ${money(usage.navigation.estimatedCost)}`:"no habilitado · sin costo registrado"}</p></div>}</section>}
    {tab==="import"&&<section className="card"><h2>Importar conductores</h2><p>Solo carga información estructurada. Cada conductor debe establecer su contraseña, subir su foto y documentos y pasar por aprobación.</p><div className="import-actions"><button className="secondary" onClick={downloadTemplate}>Descargar plantilla</button><label className="primary file-button">Seleccionar CSV<input type="file" accept=".csv,text/csv" disabled={busy} onChange={event=>{const file=event.target.files?.[0];if(file)void validateCsv(file);}}/></label></div>{importPreview&&<><div className="membership-stats compact"><Card label="Filas" value={importPreview.totalRows}/><Card label="Válidas" value={importPreview.validRows} tone="positive"/><Card label="Rechazadas" value={importPreview.rejectedRows} tone="danger"/></div><div className="table-wrap"><ManagedTable><thead><tr><th>Fila</th><th>Conductor</th><th>Correo</th><th>Cooperativa</th><th>Resultado</th></tr></thead><tbody>{importPreview.items.map((item:any)=><tr key={item.rowNumber}><td>{item.rowNumber}</td><td>{item.firstNames} {item.lastNames}</td><td>{item.email}</td><td>{item.cooperativeCode}</td><td>{item.errors.length?item.errors.join(", "):"Válida"}</td></tr>)}</tbody></ManagedTable></div><button className="primary" disabled={busy||importPreview.validRows===0} onClick={()=>void confirmImport()}>Confirmar {importPreview.validRows} cuentas</button></>}</section>}
    {planEditor&&<div className="membership-modal-backdrop" role="presentation" onMouseDown={event=>{if(event.target===event.currentTarget&&!busy)setPlanEditor(null);}}><form className="membership-modal-card plan-editor-modal" role="dialog" aria-modal="true" aria-labelledby="plan-editor-title" onSubmit={event=>{event.preventDefault();void submitPlan();}}><div className="membership-modal-heading"><div><span className="eyebrow">{planEditor.mode==="create"?"NUEVO PLAN":"NUEVA VERSIÓN"}</span><h2 id="plan-editor-title">{planEditor.mode==="create"?(planEditor.draft.planType==="TRIP_PACK"?"Crear plan por viajes":"Crear plan por período"):`Actualizar ${planEditor.source.name}`}</h2></div><button type="button" className="modal-close-button" aria-label="Cerrar" disabled={busy} onClick={()=>setPlanEditor(null)}>×</button></div>{planEditor.mode==="edit"&&<p className="plan-version-note">Las membresías activas conservarán la versión {planEditor.source.version}. Solo las nuevas compras usarán estos valores.</p>}<div className="plan-form-grid"><label className="wide">Nombre visible<input autoFocus required minLength={3} value={planEditor.draft.name} onChange={event=>setPlanEditor({...planEditor,draft:{...planEditor.draft,name:event.target.value}})} placeholder={planEditor.draft.planType==="TRIP_PACK"?"Ej. Paquete 25 viajes":"Ej. Plan de 5 meses"}/></label>{planEditor.mode==="create"&&<label className="wide">Código interno (opcional)<input minLength={3} value={planEditor.draft.code} onChange={event=>setPlanEditor({...planEditor,draft:{...planEditor.draft,code:event.target.value.toUpperCase().replace(/[^A-Z0-9_]/g,"")}})} placeholder="Se genera automáticamente"/><small>Solo se usa internamente; no lo verá el conductor.</small></label>}{planEditor.draft.planType==="PERIODIC"?<><label>Unidad de duración<select value={planEditor.draft.periodUnit} onChange={event=>setPlanEditor({...planEditor,draft:{...planEditor.draft,periodUnit:event.target.value as PlanEditorState["draft"]["periodUnit"]}})}><option value="DAY">Días</option><option value="MONTH">Meses</option><option value="QUARTER">Trimestres</option><option value="YEAR">Años</option></select></label><label>Cantidad<input type="number" required min="1" max="24" step="1" value={planEditor.draft.periodCount} onChange={event=>setPlanEditor({...planEditor,draft:{...planEditor.draft,periodCount:Number(event.target.value)}})}/></label><div className="plan-duration-preview wide"><strong>{planDurationDays(planEditor.draft.periodUnit,planEditor.draft.periodCount)||0} días de vigencia</strong><span>Puede crear cualquier período permitido.</span></div></>:<><label className="wide">Vigencia máxima en días (opcional)<input type="number" min="0" max="3650" step="1" value={planEditor.draft.packValidityDays} onChange={event=>setPlanEditor({...planEditor,draft:{...planEditor.draft,packValidityDays:Number(event.target.value)}})}/><small>Acepta cualquier cantidad entre 1 y 3650. Usa 0 para que los viajes no caduquen.</small></label></>}<label>Precio base (USD)<input type="number" required min="0" step="0.01" value={planEditor.draft.baseAmount} onChange={event=>setPlanEditor({...planEditor,draft:{...planEditor.draft,baseAmount:Number(event.target.value)}})}/></label><label>{planEditor.draft.planType==="TRIP_PACK"?"Cantidad de viajes":"Viajes incluidos"}<input type="number" required min={planEditor.draft.planType==="TRIP_PACK"?1:0} step="1" value={planEditor.draft.includedTrips} onChange={event=>setPlanEditor({...planEditor,draft:{...planEditor.draft,includedTrips:Number(event.target.value)}})}/></label>{planEditor.draft.planType==="PERIODIC"&&<><label>Tope de renovación (USD)<input type="number" required min={planEditor.draft.baseAmount} step="0.01" value={planEditor.draft.maxRenewalAmount} onChange={event=>setPlanEditor({...planEditor,draft:{...planEditor.draft,maxRenewalAmount:Number(event.target.value)}})}/></label><label>Participación adicional (%)<input type="number" required min="0" max="100" step="0.01" value={planEditor.draft.extraTripSharePercent} onChange={event=>setPlanEditor({...planEditor,draft:{...planEditor.draft,extraTripSharePercent:Number(event.target.value)}})}/></label></>}</div><div className="membership-modal-actions"><button type="button" className="secondary" disabled={busy} onClick={()=>setPlanEditor(null)}>Cancelar</button><button type="submit" className="primary" disabled={busy}>{busy?"Guardando…":planEditor.mode==="create"?"Crear plan":"Publicar nueva versión"}</button></div></form></div>}
