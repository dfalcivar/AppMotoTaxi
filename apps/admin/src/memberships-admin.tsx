...
function MembershipManagement({token,permissions}:Props) {
  ...
  return <div className="membership-admin">
    ...
    {tab==="payments"&&<section className="card"><h2>Transferencias por verificar</h2><p className="muted">Contrasta el monto y la referencia con el movimiento real de la cuenta bancaria. Estas transferencias no requieren cierre de caja ni conciliación posterior.</p><div className="table-wrap"><table><thead><tr><th>Conductor</th><th>Referencia bancaria</th><th>Declarado</th><th>Esperado</th><th>Fecha</th><th>Acción</th></tr></thead><tbody>{pendingPayments.map(item=><tr key={item.id}><td>{item.driver}</td><td><strong>{item.reference}</strong></td><td>{money(item.declaredAmount,item.currency)}</td><td>{money(item.expectedAmount,item.currency)}</td><td>{date(item.createdAt)}</td><td><div className="row-actions"><button className="link" onClick={()=>setFiscalOrder(item.orderId)}>Datos fiscales</button><button className="link" disabled={busy} onClick={()=>void openPaymentProof(item.id)}>Ver comprobante</button><button className="link" disabled={busy} onClick={()=>void reviewPayment(item,true)}>Verificar y aprobar</button><button className="link danger" disabled={busy} onClick={()=>void reviewPayment(item,false)}>Rechazar</button></div></td></tr>)}</tbody></table></div>{!pendingPayments.length&&<p className="empty">No existen comprobantes pendientes de verificación bancaria.</p>}</section>}
