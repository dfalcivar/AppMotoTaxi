...
function MembershipManagement({token,permissions}:Props) {
  ...
  async function editCollectionPoint(point:any){
    const values=await dialog.open({title:"Configurar punto de recaudación",description:"Define su estado y los métodos que podrá confirmar el personal asignado.",confirmLabel:"Guardar punto",fields:[{name:"name",label:"Nombre",initialValue:String(point.name??""),required:true,minLength:3},{name:"status",label:"Estado",type:"select",initialValue:String(point.status??"INACTIVE"),required:true,options:[{value:"ACTIVE",label:"Activo"},{value:"SUSPENDED",label:"Suspendido"},{value:"INACTIVE",label:"Inactivo"}]},{name:"cashEnabled",label:"Aceptar efectivo",type:"checkbox",initialValue:Boolean(point.cashEnabled)},{name:"deunaEnabled",label:"Aceptar De Una",type:"checkbox",initialValue:Boolean(point.deunaEnabled)},{name:"bankTransferEnabled",label:"Aceptar transferencia verificada",type:"checkbox",initialValue:Boolean(point.bankTransferEnabled)}]});if(!values)return;
    setBusy(true);try{await apiFetch(`/v1/admin/collection-points/${point.id}`,token,{method:"PATCH",body:JSON.stringify({name:String(values.name).trim(),address:point.address??null,serviceAreaId:point.serviceAreaId??null,status:String(values.status),cashEnabled:Boolean(values.cashEnabled),deunaEnabled:Boolean(values.deunaEnabled),bankTransferEnabled:Boolean(values.bankTransferEnabled),settlementDeadlineHours:Number(point.settlementDeadlineHours??48),pendingLimit:point.pendingLimit??null})});setSuccess("Punto actualizado y auditado.");await load();}catch(reason){setError(reason instanceof Error?reason.message:"No se pudo actualizar.");}finally{setBusy(false);}
  }
  async function assignCollector(point:any){
    ...
    const values=await dialog.open({title:"Asignar recaudador",description:`Selecciona quién podrá confirmar pagos en ${point.name}.`,confirmLabel:"Asignar",fields:[{name:"collectorId",label:"Recaudador",type:"select",initialValue:String(available[0].id),required:true,options:available.map(item=>({value:String(item.id),label:`${item.name} · ${item.email}`}))}]});if(!values)return;const collector=available.find(item=>String(item.id)===String(values.collectorId));if(!collector)return;
