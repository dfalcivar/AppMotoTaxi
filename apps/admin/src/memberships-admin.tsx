...
function MembershipManagement({token,permissions}:Props) {
  ...
  async function courtesyRenew(driverId:string){
    ...
    const options=enabledPlans.map((plan,index)=>`${index+1}. ${plan.name} (${money(plan.baseAmount,plan.currency)})`).join("\n");
    const selected=Number(window.prompt(`Selecciona el plan que se activará sin cobro:\n${options}`,"1"));
    const plan=enabledPlans[selected-1];if(!plan)return;
    const reason=window.prompt("Motivo obligatorio de la cortesía");if(!reason||reason.trim().length<5)return;
    if(!window.confirm(`¿Activar el plan ${plan.name} como cortesía? La operación quedará auditada y no registrará ingreso.`))return;
    setBusy(true);setError("");
  ...
  function requestPlanValues(initial?:any){
    const name=window.prompt("Nombre visible del plan",initial?.name??"Cuatro meses");if(!name)return;
    const durationDays=Number(window.prompt("Duración en días",String(initial?.durationDays??120)));
    const baseAmount=Number(window.prompt("Precio base USD",String(initial?.baseAmount??10)));
    const includedTrips=Number(window.prompt("Viajes incluidos",String(initial?.includedTrips??100)));
    const maximum=Number(window.prompt("Tope máximo de renovación USD",String(initial?.maxRenewalAmount??baseAmount)));
    const extraTripSharePercent=Number(window.prompt("Participación por viajes adicionales (%)",String(initial?.extraTripSharePercent??settings?.membershipExtraTripSharePercent??40)));
    if(!Number.isInteger(durationDays)||durationDays<1||durationDays>730||!Number.isFinite(baseAmount)||baseAmount<0||!Number.isInteger(includedTrips)||includedTrips<0||!Number.isFinite(maximum)||maximum<baseAmount||!Number.isFinite(extraTripSharePercent)||extraTripSharePercent<0||extraTripSharePercent>100){setError("Revisa duración, precio, viajes incluidos, tope y participación adicional.");return;}
    const periodUnit=durationDays<=24?"DAY":durationDays===365?"YEAR":durationDays===90?"QUARTER":"MONTH";
    const periodCount=periodUnit==="DAY"?durationDays:periodUnit==="YEAR"||periodUnit==="QUARTER"?1:Math.max(1,Math.min(24,Math.round(durationDays/30)));
    return {name,periodUnit,periodCount,durationDays,baseAmount,currency:"USD",includedTrips,maxRenewalAmount:maximum,extraTripSharePercent};
  }

  async function createPlan(){
    const values=requestPlanValues();if(!values)return;
    const suggested=values.name.normalize("NFD").replace(/[\u0300-\u036f]/g,"").toUpperCase().replace(/[^A-Z0-9]+/g,"_");
    const code=window.prompt("Código lógico único",suggested)?.trim().toUpperCase();if(!code)return;
    setBusy(true);setError("");try{await apiFetch("/v1/admin/membership-plans",token,{method:"POST",body:JSON.stringify({...values,code,enabled:true})});setSuccess("Plan creado. Los ciclos existentes conservan sus condiciones.");await load();}catch(reason){setError(reason instanceof Error?reason.message:"No se pudo crear el plan.");}finally{setBusy(false);}
  }

  async function editPlan(plan:any){
    const values=requestPlanValues(plan);if(!values)return;
    if(!window.confirm(`¿Publicar una nueva versión de ${plan.name}? Las membresías activas conservarán la versión ${plan.version} hasta finalizar su ciclo.`))return;
    setBusy(true);setError("");try{await apiFetch(`/v1/admin/membership-plans/${plan.id}/versions`,token,{method:"POST",body:JSON.stringify(values)});setSuccess(`Nueva versión de ${plan.name} publicada. Las membresías vigentes no fueron modificadas.`);await load();}catch(reason){setError(reason instanceof Error?reason.message:"No se pudo versionar el plan.");}finally{setBusy(false);}
  }

  async function deactivatePlan(plan:any){
    const reason=window.prompt(`Motivo para desactivar ${plan.name}`);if(!reason||reason.trim().length<5)return;
    if(!window.confirm(`¿Desactivar ${plan.name}? Ya no aparecerá para nuevas activaciones, pero los ciclos vigentes continuarán normalmente.`))return;
    setBusy(true);setError("");try{await apiFetch(`/v1/admin/membership-plans/${plan.id}/deactivate`,token,{method:"POST",body:JSON.stringify({reason})});setSuccess(`${plan.name} fue desactivado sin afectar membresías vigentes.`);await load();}catch(reason){setError(reason instanceof Error?reason.message:"No se pudo desactivar el plan.");}finally{setBusy(false);}
  }
  ...
  async function editCollectionPoint(point:any){
    ...
    const statusChoice=window.prompt("Estado: ACTIVO, SUSPENDIDO o INACTIVO",labelFor(operationalStatusLabels,point.status))?.trim().toUpperCase();
    const status=({ACTIVO:"ACTIVE",SUSPENDIDO:"SUSPENDED",INACTIVO:"INACTIVE",ACTIVE:"ACTIVE",SUSPENDED:"SUSPENDED",INACTIVE:"INACTIVE"} as Record<string,string>)[statusChoice??""];if(!status)return;
    const currentMethods=[point.cashEnabled&&"EFECTIVO",point.deunaEnabled&&"DE UNA",point.bankTransferEnabled&&"TRANSFERENCIA"].filter(Boolean).join(", ");
    const methodChoices=window.prompt("Métodos separados por coma: EFECTIVO, DE UNA, TRANSFERENCIA",currentMethods)?.toUpperCase()??"";
    const methods=[methodChoices.includes("EFECTIVO")&&"CASH",methodChoices.includes("DE UNA")&&"DEUNA",methodChoices.includes("TRANSFERENCIA")&&"BANK_TRANSFER"].filter(Boolean).join(",");
