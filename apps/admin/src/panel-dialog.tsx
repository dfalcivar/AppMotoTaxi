...
export type PanelDialogValue = string | number | boolean | File | { latitude:number; longitude:number } | PanelDialogScheduleValue;
export type PanelDialogField = {
  name: string;
  label: string;
  type?: "text" | "number" | "textarea" | "select" | "checkbox" | "map-point" | "schedule-range" | "file";
...
export function usePanelDialog() {
  ...
  function submit(event:React.FormEvent){
    ...
    for(const field of config?.fields??[]){
      ...
      if(field.required&&(field.type==="checkbox"?!value:field.type==="file"?!(value instanceof File):!text)){setError(`Completa «${field.label}».`);return;}
      if(field.type==="file"&&value instanceof File&&(value.type!=="application/pdf"||value.size>3*1024*1024)){setError(`«${field.label}» debe ser un PDF de hasta 3 MB.`);return;}
