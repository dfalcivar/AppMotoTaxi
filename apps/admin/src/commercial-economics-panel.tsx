...
export function CommercialEconomicsPanel({token,permissions,mode}:{token:string;permissions:string[];mode:'settings'|'packages'|'dashboard'}) {
  ...
  function configure(){
    ...
    setConfigurationDraft({enabled:Boolean(config.enabled),preserveCancelledSearchRound:config.preserveCancelledSearchRound!==false,costaGoPercent:String(data.settings.costaGoPercent??''),searchSessionMinutes:String(config.searchSessionMinutes??''),
  ...
  return <section className={`card${mode==='settings'?' economic-summary-shell':''}`}>
    ...
    {mode==='settings'&&data&&<div className="economic-summary">
      ...
      <div className="economic-summary-notice"><b aria-hidden="true">ⓘ</b><span><strong>{data.settings.configuration?.enabled?'Activo para nuevas operaciones.':'El modelo está desactivado.'}</strong> Los ciclos existentes conservan sus condiciones.</span></div>
      <div className="economic-summary-groups">
        ...
        <section className="economic-summary-group"><div className="economic-summary-label"><span aria-hidden="true">$</span><div><h3>Valor por ronda</h3><p>Tarifa y participación<br/>de Costa-Go.</p></div></div><div className="economic-summary-metrics two"><article><strong>{money(data.settings.feePerRound)}</strong><small>Valor por ronda</small></article><article><strong>{Number(data.settings.costaGoPercent).toFixed(2)}%</strong><small>Participación Costa-Go</small></article></div><p className="economic-summary-hint"><b aria-hidden="true">ⓘ</b>Las rondas siguientes se calculan automáticamente.</p></section>
