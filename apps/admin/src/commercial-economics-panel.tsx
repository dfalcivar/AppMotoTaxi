...
type EconomicsDraft={enabled:boolean;preserveCancelledSearchRound:boolean;costaGoPercent:string;maxCommissionPerTrip:string;searchSessionMinutes:string;sameRouteToleranceMeters:string;
...
export function CommercialEconomicsPanel({token,permissions,mode}:{token:string;permissions:string[];mode:'settings'|'packages'|'dashboard'}) {
  ...
  function configure(){
    ...
    setConfigurationDraft({enabled:Boolean(config.enabled),preserveCancelledSearchRound:config.preserveCancelledSearchRound!==false,costaGoPercent:String(data.settings.costaGoPercent??''),maxCommissionPerTrip:String(config.maxCommissionPerTrip??'0.25'),searchSessionMinutes:String(config.searchSessionMinutes??''),
  ...
  async function saveConfiguration(event:React.FormEvent){
    ...
    if(!/^\d{1,6}(?:\.\d{1,2})?$/.test(d.maxCommissionPerTrip)){setConfigurationError('El techo de comisión debe ser un importe válido en USD.');return;}
    ...
    const configuration={...current,enabled:d.enabled,preserveCancelledSearchRound:d.preserveCancelledSearchRound,
      maxCommissionPerTrip:Number(d.maxCommissionPerTrip).toFixed(2),
  ...
  const draftFee=Number(data?.settings?.feePerRound??0),draftShare=Number(configurationDraft?.costaGoPercent??0),
    draftCap=Number(configurationDraft?.maxCommissionPerTrip??0);
  ...
  return <section className={`card${mode==='settings'?' economic-summary-shell':''}${mode==='dashboard'?' economic-dashboard-panel':''}`}>
    ...
    {mode==='settings'&&data&&<div className="economic-summary">
      <header className="economic-summary-heading"><span className="economic-summary-main-icon" aria-hidden="true">●</span><div><h2>Tarifa de llegada y saldo Costa-Go</h2><p>Resumen del modelo económico</p></div><span className={`economic-summary-status ${data.settings.configuration?.enabled?'active':'inactive'}`}><i aria-hidden="true"/>{data.settings.configuration?.enabled?'Modelo activo':'Modelo inactivo'}</span></header>
      <div className="economic-summary-notice"><b aria-hidden="true">ⓘ</b><span><strong>{data.settings.configuration?.enabled?'Activo para nuevas operaciones.':'El modelo está desactivado.'}</strong> Las cotizaciones y viajes ya creados conservan sus importes.</span></div>
      <div className="economic-summary-groups">
        <section className="economic-summary-group"><div className="economic-summary-label"><span aria-hidden="true">⌖</span><div><h3>Despacho</h3><p>Configuración de distancia<br/>y tiempo por ronda.</p></div></div><div className="economic-summary-metrics four"><article><strong>{data.settings.initialRadiusMeters} m</strong><small>iniciales</small></article><article><small>incremento</small><strong>{data.settings.radiusIncrementMeters} m</strong></article><article><small>máximo</small><strong>{data.settings.maximumRadiusMeters} m</strong></article><article><strong>{data.settings.roundWaitSeconds} s</strong><small>por ronda</small></article></div></section>
        <section className="economic-summary-group"><div className="economic-summary-label"><span aria-hidden="true">$</span><div><h3>Valor por ronda</h3><p>Tarifa y participación<br/>de Costa-Go.</p></div></div><div className="economic-summary-metrics three"><article><strong>{money(data.settings.feePerRound)}</strong><small>Valor por ronda</small></article><article><strong>{Number(data.settings.costaGoPercent).toFixed(2)}%</strong><small>Participación Costa-Go</small></article><article><strong>{money(data.settings.configuration?.maxCommissionPerTrip)}</strong><small>Techo por viaje inmediato</small></article></div><p className="economic-summary-hint"><b aria-hidden="true">ⓘ</b>El techo solo limita la comisión en viajes inmediatos; no cambia el total del pasajero ni los programados.</p></section>
        <section className="economic-summary-group"><div className="economic-summary-label"><span aria-hidden="true">▥</span><div><h3>Llegada y saldo</h3><p>Límites por viaje.</p></div></div><div className="economic-summary-metrics three"><article><strong>{data.settings.rounds??'—'}</strong><small>rondas</small></article><article><strong>{money(data.settings.minimumArrival)}</strong><small>Llegada mínima</small></article><article><strong>{money(data.settings.maximumArrival)}</strong><small>máxima por viaje</small></article></div></section>
      </div>
      <button className="economic-summary-configure" disabled={busy||!can('settings:manage')} onClick={configure}><span aria-hidden="true">⚙</span>Configurar modelo económico <b aria-hidden="true">→</b></button>
    </div>}
    ...
    {configurationDraft&&<div className="economic-modal-backdrop" role="presentation" onMouseDown={event=>{if(event.target===event.currentTarget&&!busy)setConfigurationDraft(null);}}>
      <form className="economic-modal" role="dialog" aria-modal="true" aria-labelledby="economic-modal-title" onSubmit={event=>void saveConfiguration(event)}>
        ...
        <section className="economic-activation"><label className="economic-toggle"><input type="checkbox" checked={configurationDraft.enabled} onChange={event=>updateDraft('enabled',event.target.checked)}/><i aria-hidden="true"/><span><strong>Activar modelo para nuevas solicitudes y ciclos</strong><small>Las cotizaciones y viajes ya creados conservan sus importes.</small></span></label><p><b>ⓘ</b><span>El techo aplica solo a viajes inmediatos nuevos.<small>No modifica precios publicados automáticamente.</small></span></p></section>
        <section className="economic-rate-card"><div className="economic-section-heading"><span className="economic-section-icon">$</span><div><h3>Tarifa de llegada por rondas</h3><p>Ingresa una sola tarifa base; no es necesario configurar cada ronda.</p></div></div><div className="economic-rate-layout">
          ...
          {input('maxCommissionPerTrip','Techo máximo de comisión Costa-Go por viaje inmediato','Solo limita la comisión de viajes por rondas; no cambia el total del pasajero ni los programados.',{prefix:'$',min:0,step:'.01'})}
          <div className="economic-round-preview"><span>Vista previa automática</span><div>{displayedRounds.map((round,index)=><article key={round} className={index===displayedRounds.length-1&&totalRounds>6?'last-round':''}><small>Ronda {round}</small><strong>{money(draftFee*round)}</strong><em>Costa-Go {money(Math.min(draftFee*round*draftShare/100,draftCap))}</em></article>)}</div>{totalRounds>6&&<small>Se muestran las primeras rondas y la ronda máxima configurada.</small>}</div>
        </div></section>
        <div className="economic-config-grid">
          <section className="economic-config-card"><div className="economic-section-heading"><span className="economic-section-icon">◷</span><div><h3>Parámetros operativos</h3><p>Controlan la continuidad de una misma búsqueda.</p></div></div><div className="economic-fields">{input('searchSessionMinutes','Ventana de la búsqueda','Tiempo durante el cual se reconoce el mismo trayecto.',{min:1,max:1440})}{input('sameRouteToleranceMeters','Tolerancia del trayecto','Distancia permitida para considerar que es la misma ruta.',{min:1,max:1000})}</div><label className="economic-search-continuity economic-toggle"><input type="checkbox" checked={configurationDraft.preserveCancelledSearchRound} onChange={event=>updateDraft('preserveCancelledSearchRound',event.target.checked)}/><i aria-hidden="true"/><span><strong>Conservar ronda después de cancelar</strong><small>Activado: la siguiente búsqueda continúa desde la ronda alcanzada. Desactivado: vuelve a comenzar desde la ronda 1.</small></span></label></section>
        ...
        <aside className="economic-notice"><b>ⓘ</b><span><strong>Información importante</strong><small>Los cambios crean una nueva versión económica para cotizaciones futuras. Los viajes anteriores permanecen congelados.</small></span></aside>
