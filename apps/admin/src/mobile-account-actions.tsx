...
export function MobileAccountActions({token,account,canEdit,canDelete=false,onChanged}:{token:string;account:Account;canEdit:boolean;canDelete?:boolean;onChanged:(message:string)=>Promise<void>}) {
  ...
  return <>{canEdit&&<button type="button" className="link" onClick={()=>open('edit')}>Editar datos</button>}{canDelete&&incomplete&&<button type="button" className="link" onClick={()=>open('delete')}>Eliminar registro incompleto</button>}
    {mode&&<div className="panel-dialog-backdrop"><form className="panel-dialog-card mobile-account-dialog" role="dialog" aria-modal="true" aria-label={mode==='edit'?'Editar datos de la cuenta':'Eliminar registro incompleto'} onSubmit={submit}>
      <div className="panel-dialog-heading"><div><span className="eyebrow">CUENTA COSTA-GO</span><h2>{mode==='edit'?'Editar datos':'Eliminar registro incompleto'}</h2><p>{account.name}</p></div><button type="button" className="secondary" disabled={busy} onClick={()=>setMode(null)}>Cerrar</button></div>
      <div className="panel-dialog-fields" style={{gridTemplateColumns:'minmax(0,1fr)'}}>
      {mode==='edit'?<><p className="note">Confirma los datos con el titular. Cambiar el correo exige verificar la nueva dirección y cierra las sesiones anteriores. No aprueba al conductor ni levanta suspensiones.</p>
        <p>Correo: {account.emailVerifiedAt?'Verificado':'Pendiente de verificación'}</p>
        <label>Nombre completo<input autoFocus required disabled={busy} minLength={3} maxLength={120} value={form.name} onChange={e=>setForm({...form,name:e.target.value})}/></label>
        <label>Correo electrónico<input required type="email" disabled={busy} maxLength={254} value={form.email} onChange={e=>setForm({...form,email:e.target.value})}/></label>
        <label>Teléfono<input required type="tel" disabled={busy} maxLength={30} placeholder="0991234567 o +593991234567" value={form.phone} onChange={e=>setForm({...form,phone:e.target.value})}/></label>
      ...
      <label>Motivo obligatorio<textarea required disabled={busy} minLength={5} maxLength={500} value={form.reason} onChange={e=>setForm({...form,reason:e.target.value})} placeholder="Por ejemplo: correo incorrecto confirmado con el titular"/></label></div>
