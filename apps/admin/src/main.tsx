...
const icons: Record<Module, string> = { dashboard: "▦", trips: "↔", drivers: "◉", passengers: "◎", cooperatives: "⌂", pricing: "$", zones: "◇", settings: "⌖", advertising: "▣", incidents: "!", access: "⚿", audit: "≡", database: "◫" };
...
function Dashboard({ token, cooperative = false }: { token: string; cooperative?: boolean }) {
  ...
  useEffect(() => { apiFetch<any>(cooperative ? "/v1/admin/cooperative-dashboard/summary" : "/v1/admin/dashboard", token).then(setData).catch(reason => setError(errorText(reason))); }, [token, cooperative]);
  if (error) return <Notice error={error} />; if (!data) return <Empty text="Cargando operación…" />;
  const metricLabels: Record<string, string> = { activeTrips: "Viajes activos", availableDrivers: "Conductores disponibles", pendingDrivers: "Por aprobar", openIncidents: "Incidentes abiertos", totalTrips: "Viajes totales", completedTrips: "Completados", cancelledTrips: "Cancelados", passengersServed: "Pasajeros atendidos", activeDrivers: "Conductores activos" };
  const metrics = cooperative ? Object.fromEntries(Object.entries(data).filter(([key]) => key in metricLabels)) : data.metrics;
  return <><div className="metric-grid">{Object.entries(metrics).map(([key, value]) => <article className="metric" key={key}><span>{metricLabels[key] ?? key}</span><strong>{String(value)}</strong></article>)}</div><section className="card"><Header eyebrow={cooperative ? "ALCANCE RESTRINGIDO" : "OPERACIÓN EN VIVO"} title={cooperative ? data.cooperative : "Resumen del piloto"} action="Conectado a Render" /><p className="note">{cooperative ? "Estas cifras contienen únicamente información agregada de tu cooperativa." : "Administra viajes, aprobaciones, tarifas, zonas y el radio de búsqueda desde el menú lateral."}</p></section></>;
}
...
function DriverDocuments({ token, driver, canManage }: { token: string; driver: any; canManage: boolean }) {
  const labels: Record<string, string> = { PROFILE_PHOTO: "Foto del conductor", LICENSE: "Licencia de conducir", REGISTRATION: "Matrícula de la mototaxi", OPERATING_PERMIT: "Permiso de operación" };
  ...
  async function review(item: any, status: "ACTIVE" | "REJECTED") { const note = prompt(status === "ACTIVE" ? "Observación de aprobación:" : "Motivo del rechazo:", status === "ACTIVE" ? "Documento verificado" : "Documento ilegible o incompleto"); if (!note?.trim()) return; setBusy(item.id); try { await apiFetch(`/v1/admin/drivers/${driver.id}/documents/${item.id}`, token, { method: "PATCH", body: JSON.stringify({ status, note: note.trim() }) }); await load(); } catch (reason) { setError(errorText(reason)); } finally { setBusy(""); } }
  async function deactivate(item: any) { if (item.documentType === "PROFILE_PHOTO") { setError("La foto obligatoria no puede desactivarse; reemplázala por una nueva."); return; } if (!confirm(`Desactivar ${labels[item.documentType]}?`)) return; setBusy(item.id); try { await apiFetch(`/v1/admin/drivers/${driver.id}/documents/${item.id}`, token, { method: "DELETE" }); await load(); } catch (reason) { setError(errorText(reason)); } finally { setBusy(""); } }
  ...
  return <><button className="link" type="button" onClick={load}>Administrar documentos</button>{open && <div className="modal-backdrop" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget) setOpen(false); }}><section className="modal-card document-modal" role="dialog" aria-modal="true"><Header eyebrow="IDENTIDAD Y HABILITANTES" title={driver.name} action={driver.email} /><Notice error={error} /><div className="document-grid">{Object.entries(labels).map(([type, label]) => { const item = items.find(value => value.documentType === type); return <article className="document-item" key={type}>{item?.fileBase64 ? <img src={`data:${item.fileMime};base64,${item.fileBase64}`} alt={label} /> : <div className="document-placeholder">Sin archivo</div>}<strong>{label}</strong>{item ? <><Badge value={item.status} /><small>Cargado: {new Date(item.createdAt).toLocaleString()}</small>{item.expiresAt && <small>Vence: {new Date(item.expiresAt).toLocaleDateString()}</small>}{item.reviewNote && <small>{item.reviewNote}</small>}<div className="row-actions"><button className="link" onClick={() => openFile(item)}>Abrir</button><button className="link" onClick={() => openFile(item, true)}>Descargar</button></div></> : <small>Pendiente de cargar</small>}{canManage && <><label className="file-action">{item ? "Reemplazar" : "Cargar"}<input type="file" accept="image/jpeg,image/png,image/webp" disabled={Boolean(busy)} onChange={event => upload(type, event.target.files?.[0])} /></label>{item && <div className="row-actions"><button className="link" disabled={busy === item.id} onClick={() => review(item, "ACTIVE")}>Aprobar</button><button className="link danger" disabled={busy === item.id} onClick={() => review(item, "REJECTED")}>Rechazar</button><button className="link danger" disabled={busy === item.id} onClick={() => deactivate(item)}>Desactivar</button></div>}</>}</article>; })}</div><button className="secondary" onClick={() => setOpen(false)}>Cerrar</button></section></div>}</>;
}
...
function Drivers({ token, canApprove, canViewDocuments, canManageDocuments, canResetPasswords }: { token: string; canApprove: boolean; canViewDocuments: boolean; canManageDocuments: boolean; canResetPasswords: boolean }) {
  ...
  return <section className="card"><Header eyebrow="VALIDACIÓN" title="Conductores" action={`${data.filter(d => d.status === "PENDING").length} pendientes`} /><Notice error={error} />{data.length ? <Table headers={["Nombre", "Correo", "Teléfono", "Vehículo", "Cooperativa", "Documentos", "Estado", "De Una", "Acción"]} rows={data.map(driver => [driver.name, driver.email ?? "Sin correo", driver.phone, driver.vehicle, canApprove ? <select aria-label={`Cooperativa de ${driver.name}`} value={driver.cooperativeId ?? ""} disabled={busy === driver.id} onChange={event => update(driver, driver.status, Boolean(driver.deunaEnabled), event.target.value || null)}><option value="">Sin cooperativa</option>{cooperatives.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select> : driver.cooperativeName ?? "Sin cooperativa", <div><span>{driver.documents}</span>{canViewDocuments && <DriverDocuments token={token} driver={driver} canManage={canManageDocuments} />}</div>, <Badge value={driver.status} />, canApprove ? <input aria-label={`De Una para ${driver.name}`} type="checkbox" checked={Boolean(driver.deunaEnabled)} disabled={busy === driver.id} onChange={e => update(driver, driver.status, e.target.checked)} /> : "—", canApprove || canResetPasswords ? <div className="row-actions">{canApprove && <select aria-label={`Estado de ${driver.name}`} value={driver.status} disabled={busy === driver.id} onChange={e => update(driver, e.target.value)}><option value="PENDING">Pendiente</option><option value="ACTIVE">Aprobar / activar</option><option value="SUSPENDED">Suspender</option><option value="REJECTED">Rechazar</option></select>}{canResetPasswords && <PasswordReset token={token} userId={driver.id} userName={driver.name} />}</div> : "Solo lectura"])} /> : <Empty text="No hay conductores registrados." />}</section>;
}
