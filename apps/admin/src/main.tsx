...
function MobileAccessManagement({token}:{token:string}) {
  const [users,setUsers]=useState<any[]>([]);const [error,setError]=useState("");const [success,setSuccess]=useState("");const [busy,setBusy]=useState("");const [search,setSearch]=useState("");const [accessAction,setAccessAction]=useState<{user:any;enabled:boolean}>();
  const load=async()=>{try{setUsers(await apiFetch<any[]>("/v1/admin/mobile-access/users",token));}catch(reason){setError(errorText(reason));}};
  useEffect(()=>{void load();},[token]);
  async function update(reason:string){if(!accessAction)return;const {user,enabled}=accessAction;setBusy(user.id);setError("");setSuccess("");try{await apiFetch(`/v1/admin/mobile-access/users/${user.id}`,token,{method:"PUT",body:JSON.stringify({enabled,role:user.role??"MOBILE_OPERATIONS_ADMIN",reason})});setSuccess(`${enabled?"Acceso móvil concedido a":"Acceso móvil revocado para"} ${user.name}.`);setAccessAction(undefined);await load();}catch(value){setError(errorText(value));}finally{setBusy("");}}
  const shown=users.filter(user=>`${user.name} ${user.email}`.toLowerCase().includes(search.trim().toLowerCase()));
  return <><section className="card"><Header eyebrow="ADMINISTRACIÓN MÓVIL" title="Acceso desde la aplicación" action={`${users.filter(user=>user.enabled).length} autorizados`}/><p className="note">Este permiso es adicional: la persona conserva su perfil de pasajero o conductor. La aplicación solo habilita módulos operativos y nunca expone credenciales ni secretos.</p><div className="module-toolbar"><input aria-label="Buscar cuenta móvil" value={search} onChange={event=>setSearch(event.target.value)} placeholder="Buscar pasajero o conductor…"/></div><Notice error={error} success={success}/>{shown.length?<Table headers={["Cuenta móvil","Perfil","Nivel","Acceso","Acción"]} rows={shown.map(user=>[<div><strong>{user.name}</strong><small>{user.email}</small></div>,(user.mobileRoles??[]).join(" / "),<select aria-label={`Nivel de ${user.name}`} value={user.role??"MOBILE_OPERATIONS_ADMIN"} disabled={user.enabled||busy===user.id} onChange={event=>setUsers(current=>current.map(item=>item.id===user.id?{...item,role:event.target.value}:item))}><option value="MOBILE_OPERATIONS_ADMIN">Operaciones móvil</option><option value="SUPER_ADMIN">Superadministración móvil</option></select>,user.enabled?<Badge value="ACTIVE"/>:<Badge value="INACTIVE"/>,<button className={user.enabled?"secondary":"primary"} disabled={busy===user.id} onClick={()=>{setError("");setAccessAction({user,enabled:!user.enabled});}}>{busy===user.id?"Guardando…":user.enabled?"Revocar":"Autorizar"}</button>])}/>:<Empty text="No hay cuentas móviles que coincidan."/>}</section>{accessAction&&<DecisionDialog title={accessAction.enabled?"Autorizar administración móvil":"Revocar administración móvil"} description={<><strong>{accessAction.user.name}</strong> conservará su perfil de {(accessAction.user.mobileRoles??[]).join(" / ").toLowerCase()||"usuario"}. {accessAction.enabled?<>Se habilitará el nivel <strong>{accessAction.user.role==="SUPER_ADMIN"?"Superadministración móvil":"Operaciones móvil"}</strong> dentro de la aplicación.</>:<>Su sesión administrativa móvil será cerrada y el acceso desaparecerá de la aplicación.</>}</>} fieldLabel={accessAction.enabled?"Motivo de la autorización":"Motivo de la revocación"} initialValue={accessAction.enabled?"Acceso operativo autorizado para gestión en territorio":"Acceso móvil revocado por administración"} required confirmLabel={accessAction.enabled?"Autorizar acceso":"Revocar acceso"} dangerous={!accessAction.enabled} busy={busy===accessAction.user.id} error={error} onCancel={()=>{if(!busy){setAccessAction(undefined);setError("");}}} onConfirm={update}/>}</>;
}
