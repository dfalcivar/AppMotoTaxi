...
function Cooperatives({ token, canManage }: { token: string; canManage: boolean }) {
  const [data,setData]=useState<any[]>([]);const [error,setError]=useState("");const [success,setSuccess]=useState("");const [loading,setLoading]=useState(true);const [search,setSearch]=useConsoleState("cooperatives:q",routeQuery().get("q")??"");const [selected,setSelected]=useState<any>();const [statusAction,setStatusAction]=useState<any>();const [busy,setBusy]=useState(false);
