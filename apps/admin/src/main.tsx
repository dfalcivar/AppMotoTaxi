...
function DriverApprovalInbox({ token, canViewDocuments, canManageDocuments, approvalPermissions, onChanged }: { token: string; canViewDocuments:boolean; canManageDocuments: boolean; approvalPermissions:string[]; onChanged: () => void }) {
  const canDecide=(permission:string)=>approvalPermissions.includes('drivers:approve')||approvalPermissions.includes(permission);
...
function Drivers({token,canApprove,approvalPermissions,canViewDocuments,canManageDocuments,canResetPasswords,canEditAccounts,canDeleteIncomplete}:{token:string;canApprove:boolean;approvalPermissions:string[];canViewDocuments:boolean;canManageDocuments:boolean;canResetPasswords:boolean;canEditAccounts:boolean;canDeleteIncomplete:boolean}) {
  ...
  return <>
    ...
    {canReview&&<details className="cg-insights"><summary>Revisar solicitudes de aprobación y documentos pendientes</summary><DriverApprovalInbox token={token} approvalPermissions={approvalPermissions} canViewDocuments={canViewDocuments} canManageDocuments={canManageDocuments} onChanged={()=>void load()}/></details>}
...
function MobileAccessManagement({token}:{token:string}) {
  type AccessRole="MOBILE_OPERATIONS_ADMIN"|"SUPER_ADMIN";
  type AccessAction={user:any;enabled:boolean;role:AccessRole;kind:"grant"|"revoke"|"edit"};
  const [users,setUsers]=useState<any[]>([]),[candidates,setCandidates]=useState<any[]>([]),[history,setHistory]=useState<any[]>([]);
  const [error,setError]=useState(""),[success,setSuccess]=useState(""),[busy,setBusy]=useState(""),[search,setSearch]=useState("");
  const [selectedCandidate,setSelectedCandidate]=useState<any>(),[selectedRole,setSelectedRole]=useState<AccessRole>("MOBILE_OPERATIONS_ADMIN");
  const [accessAction,setAccessAction]=useState<AccessAction>(),[historyOpen,setHistoryOpen]=useState(false),[historyLoading,setHistoryLoading]=useState(false);
  const roleName=(role:string)=>role==="SUPER_ADMIN"?"Superadministración móvil":"Operaciones móvil";
  const profileName=(roles:string[]=[])=>(roles.map(role=>role==="DRIVER"?"Conductor":role==="PASSENGER"?"Pasajero":role).join(" / ")||"Cuenta móvil");
  const load=async()=>{try{setUsers(await apiFetch<any[]>("/v1/admin/mobile-access/users?scope=authorized",token));}catch(reason){setError(errorText(reason));}};
  useEffect(()=>{void load();},[token]);
  useEffect(()=>{
    const term=search.trim();
    if(selectedCandidate&&term===`${selectedCandidate.name} · ${selectedCandidate.email}`){setCandidates([]);return;}
    if(term.length<2){setCandidates([]);return;}
    let cancelled=false;const timer=window.setTimeout(()=>{void apiFetch<any[]>(`/v1/admin/mobile-access/users?scope=candidates&search=${encodeURIComponent(term)}`,token).then(result=>{if(!cancelled)setCandidates(result);}).catch(reason=>{if(!cancelled)setError(errorText(reason));});},250);
    return()=>{cancelled=true;window.clearTimeout(timer);};
  },[search,selectedCandidate,token]);
  async function update(reason:string){
    if(!accessAction)return;const {user,enabled,role,kind}=accessAction;setBusy(user.id);setError("");setSuccess("");
    try{await apiFetch(`/v1/admin/mobile-access/users/${user.id}`,token,{method:"PUT",body:JSON.stringify({enabled,role,reason})});
      setSuccess(kind==="grant"?`Acceso móvil concedido a ${user.name}.`:kind==="edit"?`Nivel de acceso actualizado para ${user.name}.`:`Acceso móvil revocado para ${user.name}.`);
      setAccessAction(undefined);if(kind==="grant"){setSelectedCandidate(undefined);setSearch("");setCandidates([]);}await load();
    }catch(value){setError(errorText(value));}finally{setBusy("");}
  }
  async function openHistory(){setHistoryOpen(true);setHistoryLoading(true);setError("");try{setHistory(await apiFetch<any[]>("/v1/admin/mobile-access/history",token));}catch(value){setError(errorText(value));}finally{setHistoryLoading(false);}}
  const chooseCandidate=(user:any)=>{setSelectedCandidate(user);setSearch(`${user.name} · ${user.email}`);setCandidates([]);};
  const startGrant=()=>{if(selectedCandidate)setAccessAction({user:selectedCandidate,enabled:true,role:selectedRole,kind:"grant"});};
  return <>
    <section className="card mobile-access-overview">
      <div className="mobile-access-hero"><div><span className="eyebrow">ADMINISTRACIÓN MÓVIL</span><h2>Acceso desde la aplicación</h2></div><span className="mobile-access-count"><ConsoleIcon name="users" size={18}/>{users.length} autorizados</span></div>
      <div className="mobile-access-info"><ConsoleIcon name="alert" size={20}/><span>Este permiso es adicional: la persona conserva su perfil de pasajero o conductor. La aplicación solo habilita módulos operativos y nunca expone credenciales ni secretos.</span></div>
      <section className="mobile-access-panel mobile-access-add">
        <div className="mobile-access-section-heading"><span><ConsoleIcon name="users"/></span><div><h3>Agregar nuevo acceso</h3><p>Busca un pasajero o conductor y asígnale un nivel de acceso a la aplicación.</p></div></div>
        <div className="mobile-access-add-grid">
          <div className="mobile-access-search"><label htmlFor="mobile-access-search">Cuenta móvil</label><div className="mobile-access-search-box"><ConsoleIcon name="search"/><input id="mobile-access-search" value={search} onChange={event=>{setSearch(event.target.value);setSelectedCandidate(undefined);}} placeholder="Buscar pasajero o conductor…" autoComplete="off"/></div><small>Puedes buscar por nombre, correo electrónico o teléfono.</small>{candidates.length>0&&<div className="mobile-access-results" role="listbox">{candidates.map(user=><button type="button" role="option" key={user.id} onClick={()=>chooseCandidate(user)}><strong>{user.name}</strong><small>{user.email} · {user.phone??"Sin teléfono"}</small><em>{profileName(user.mobileRoles)}</em></button>)}</div>}</div>
          <label>Nivel de acceso<select value={selectedRole} onChange={event=>setSelectedRole(event.target.value as AccessRole)}><option value="MOBILE_OPERATIONS_ADMIN">Operaciones móvil</option><option value="SUPER_ADMIN">Superadministración móvil</option></select></label>
          <button className="primary mobile-access-authorize" type="button" disabled={!selectedCandidate||Boolean(busy)} onClick={startGrant}><span>＋</span> Autorizar usuario</button>
        </div>
      </section>
      <Notice error={error} success={success}/>
      <section className="mobile-access-panel mobile-access-authorized">
        <div className="mobile-access-authorized-heading"><div className="mobile-access-section-heading"><span><ConsoleIcon name="users"/></span><div><h3>Usuarios autorizados</h3><p>Solo se muestran las personas que tienen acceso a la aplicación móvil.</p></div></div><button className="link mobile-access-history-button" type="button" onClick={()=>void openHistory()}><ConsoleIcon name="calendar" size={17}/> Ver historial de autorizaciones</button></div>
        {users.length?<div className="table-wrap mobile-access-table"><table><thead><tr><th>Cuenta móvil</th><th>Perfil</th><th>Nivel</th><th>Estado</th><th>Último acceso</th><th>Acción</th></tr></thead><tbody>{users.map(user=><tr key={user.id}><td><strong>{user.name}</strong><small>{user.email}</small></td><td>{profileName(user.mobileRoles)}</td><td>{roleName(user.role)}</td><td><Badge value="ACTIVE"/></td><td>{user.lastAccessAt?<><strong>{ecuDate(user.lastAccessAt)}</strong><small>Acceso administrativo</small></>:<span className="muted">Aún no ha ingresado</span>}</td><td><div className="mobile-access-actions"><button className="secondary" type="button" disabled={busy===user.id} onClick={()=>setAccessAction({user,enabled:true,role:user.role,kind:"edit"})}><ConsoleIcon name="settings" size={16}/> Editar acceso</button><button className="secondary destructive" type="button" disabled={busy===user.id} onClick={()=>setAccessAction({user,enabled:false,role:user.role,kind:"revoke"})}>Revocar</button></div></td></tr>)}</tbody></table></div>:<Empty text="Todavía no hay usuarios autorizados para la administración móvil."/>}
        <div className="mobile-access-footer-note"><ConsoleIcon name="alert" size={18}/><span>Aquí solo se muestran los usuarios que ya tienen acceso autorizado a la aplicación móvil.<br/>Para agregar un nuevo usuario, utiliza el buscador en la parte superior.</span></div>
      </section>
    </section>
    {accessAction&&accessAction.kind!=="edit"&&<DecisionDialog title={accessAction.enabled?"Autorizar administración móvil":"Revocar administración móvil"} description={<><strong>{accessAction.user.name}</strong> conservará su perfil de {profileName(accessAction.user.mobileRoles).toLowerCase()}. {accessAction.enabled?<>Se habilitará el nivel <strong>{roleName(accessAction.role)}</strong> dentro de la aplicación.</>:<>Su sesión administrativa móvil será cerrada y el acceso desaparecerá de la aplicación.</>}</>} fieldLabel={accessAction.enabled?"Motivo de la autorización":"Motivo de la revocación"} initialValue={accessAction.enabled?"Acceso operativo autorizado para gestión en territorio":"Acceso móvil revocado por administración"} required confirmLabel={accessAction.enabled?"Autorizar acceso":"Revocar acceso"} dangerous={!accessAction.enabled} busy={busy===accessAction.user.id} error={error} onCancel={()=>{if(!busy){setAccessAction(undefined);setError("");}}} onConfirm={update}/>}
    {accessAction?.kind==="edit"&&<ConsoleModal title="Editar acceso móvil" subtitle={accessAction.user.name} className="mobile-access-edit-modal" onClose={()=>{if(!busy)setAccessAction(undefined);}}><label>Nivel de acceso<select value={accessAction.role} onChange={event=>setAccessAction({...accessAction,role:event.target.value as AccessRole})}><option value="MOBILE_OPERATIONS_ADMIN">Operaciones móvil</option><option value="SUPER_ADMIN">Superadministración móvil</option></select></label><p className="mobile-access-modal-note">El cambio cerrará las sesiones administrativas activas para aplicar el nuevo nivel de forma segura.</p><Notice error={error}/><div className="modal-actions"><button className="secondary" type="button" disabled={Boolean(busy)} onClick={()=>setAccessAction(undefined)}>Cancelar</button><button className="primary" type="button" disabled={Boolean(busy)} onClick={()=>void update("Nivel de administración móvil actualizado")}>{busy?"Guardando…":"Guardar acceso"}</button></div></ConsoleModal>}
    {historyOpen&&<ConsoleModal title="Historial de autorizaciones" subtitle="Altas, cambios y revocaciones de acceso móvil" className="mobile-access-history-modal" onClose={()=>setHistoryOpen(false)}>{historyLoading?<LoadingState label="Cargando historial…"/>:history.length?<Table headers={["Fecha","Cuenta móvil","Acción","Nivel","Responsable","Motivo"]} rows={history.map(item=>[ecuDate(item.createdAt),<div><strong>{item.userName??"Cuenta eliminada"}</strong><small>{item.userEmail??item.userId}</small></div>,item.action==="MOBILE_ADMIN_ACCESS_GRANTED"?"Autorización / actualización":"Revocación",roleName(item.nextValue?.role??"MOBILE_OPERATIONS_ADMIN"),item.actorName??"Sistema",item.reason??"Sin motivo"])}/>:<Empty text="Aún no existen autorizaciones registradas."/>}<div className="modal-actions"><button className="primary" type="button" onClick={()=>setHistoryOpen(false)}>Cerrar</button></div></ConsoleModal>}
  </>;
}
...
function AccessManagement({ token, currentUserId }: { token: string; currentUserId?: string }) {
  ...
  const [form, setForm] = useState({ fullName: "", email: "", phone: "", password: "", role: "SOPORTE", customRoleId: "", cooperativeId: "" });
  async function load() { try { const [nextUsers, nextRoles, nextCooperatives,nextCustomRoles] = await Promise.all([apiFetch<any[]>("/v1/admin/access/users", token), apiFetch<any[]>("/v1/admin/access/roles", token), apiFetch<any[]>("/v1/admin/cooperatives", token),apiFetch<any[]>("/v1/admin/access/custom-roles",token)]); setUsers(nextUsers); setRoles(nextRoles); setCooperatives(nextCooperatives);setCustomRoles(nextCustomRoles); } catch (reason) { setError(errorText(reason)); } }
  useEffect(() => { void load(); }, [token]);
  async function save(user: any) { setBusy(user.id); setError(""); setSuccess(""); try { await apiFetch(`/v1/admin/access/users/${user.id}`, token, { method: "PATCH", body: JSON.stringify({ role: user.role, customRoleId:user.customRoleId??null, cooperativeId: user.role === "ANALISTA_COOPERATIVA" ? user.cooperativeId || null : null, overrides: user.customRoleId?[]:user.overrides ?? [] }) }); setSuccess(`Acceso actualizado para ${user.name}. Deberá iniciar sesión nuevamente.`); setSelectedUser(undefined); await load(); } catch (reason) { setError(errorText(reason)); } finally { setBusy(""); } }
  async function create(event: React.FormEvent) { event.preventDefault(); const policyError = strongPasswordError(form.password); if (policyError) { setError(policyError); return; } setBusy("new"); setError(""); setSuccess(""); try { await apiFetch("/v1/admin/access/users", token, { method: "POST", body: JSON.stringify({ ...form, customRoleId:form.customRoleId||null, cooperativeId: form.role === "ANALISTA_COOPERATIVA" ? form.cooperativeId || null : null }) }); setForm({ fullName: "", email: "", phone: "", password: "", role: "SOPORTE", customRoleId: "", cooperativeId: "" }); setCreating(false); setSuccess("Usuario administrativo creado correctamente."); await load(); } catch (reason) { setError(errorText(reason)); } finally { setBusy(""); } }
  const filteredUsers=users.filter(user=>`${user.name} ${user.email} ${user.customRoleName??roleLabels[user.role]??user.role}`.toLowerCase().includes(search.trim().toLowerCase()));
  const selectableRoles=roles.filter(item => !["ADMIN", "SUPPORT"].includes(item.role));
  return <div className="access-module"><RolesAndPermissions token={token} roles={customRoles} onChange={load}/><MobileAccessManagement token={token}/><MetricStrip items={[
...
function App() {
  ...
  const visible=visibleAdminModules(Object.keys(labels) as Module[],allowed);
  const requested=new URLSearchParams(location).get('module') as Module;
  const currentModule=resolvedAdminModule(requested,visible);
  ...
  return <ConsoleLayout session={session} current={currentModule} labels={resolvedLabels} visible={visible} can={allowed} roleLabel={session.user.customRoleName??roleLabels[session.user.role]??session.user.role} onNavigate={selectModule} onLogout={logout} onRefresh={()=>setRevision(v=>v+1)}>
    ...
    <div key={currentModule+location+revision} className="cg-module-content"><Suspense fallback={<LoadingState label="Cargando módulo…"/>}>
      ...
      {currentModule === "drivers" && <Drivers canEditAccounts={allowed("mobile_accounts:edit")} canDeleteIncomplete={allowed("mobile_accounts:delete_incomplete")} token={session.token} canApprove={allowed("drivers:approve")} approvalPermissions={session.user.permissions??[]} canViewDocuments={allowed("drivers:documents:view")} canManageDocuments={allowed("drivers:documents:manage")} canResetPasswords={allowed("users:manage")} />}
