...
function DriverApprovalInbox({ token, canManageDocuments, onChanged }: { token: string; canManageDocuments: boolean; onChanged: () => void }) {
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener("driver-documents-changed", refresh);
  }, [token]);
...
function Settings({ token, admin }: { token: string; admin: boolean }) {
  ...
  useEffect(() => { Promise.all([apiFetch<any>("/v1/admin/settings", token), apiFetch<typeof approvalSettings>("/v1/admin/driver-approval-settings", token)]).then(([settings, approval]) => { setRadius(settings.searchRadiusMeters); setInitialRadius(settings.driverSearchInitialRadiusMeters ?? 1000); setRadiusIncrement(settings.driverSearchRadiusIncrementMeters ?? 1000); setRoundWaitSeconds(settings.driverSearchRoundWaitSeconds ?? 15); setScheduledLead(settings.scheduledTripLeadMinutes ?? 10); setScheduledMinimumNotice(settings.scheduledTripMinimumNoticeMinutes ?? 30); setDocumentExpiryDays(settings.documentExpiryAlertDays ?? 30); setApprovalSettings(approval); }).catch(reason => setError(errorText(reason))); }, [token]);
  async function save(event: React.FormEvent) { event.preventDefault(); setSuccess(""); setError(""); try { const value = await apiFetch<any>("/v1/admin/settings", token, { method: "PATCH", body: JSON.stringify({ searchRadiusMeters: radius, driverSearchInitialRadiusMeters: initialRadius, driverSearchRadiusIncrementMeters: radiusIncrement, driverSearchRoundWaitSeconds: roundWaitSeconds, scheduledTripLeadMinutes: scheduledLead, scheduledTripMinimumNoticeMinutes: scheduledMinimumNotice, documentExpiryAlertDays: documentExpiryDays }) }); setRadius(value.searchRadiusMeters); setInitialRadius(value.driverSearchInitialRadiusMeters); setRadiusIncrement(value.driverSearchRadiusIncrementMeters); setRoundWaitSeconds(value.driverSearchRoundWaitSeconds); setScheduledLead(value.scheduledTripLeadMinutes); setScheduledMinimumNotice(value.scheduledTripMinimumNoticeMinutes); setDocumentExpiryDays(value.documentExpiryAlertDays); setSuccess("Configuración operativa guardada."); } catch (reason) { setError(errorText(reason)); } }
...
function DocumentExpirySettings({ token, admin }: { token: string; admin: boolean }) {
  ...
  async function save(event: React.FormEvent) {
    ...
    try {
      ...
      const value = await apiFetch<any>("/v1/admin/settings",token,{ method:"PATCH", body:JSON.stringify({ ...current, documentExpiryAlertDays:days, updatedAt:undefined }) });
...
function AccessManagement({ token, currentUserId }: { token: string; currentUserId?: string }) {
  ...
  return <div className="split"><section className="card"><Header eyebrow="CONTROL DE ACCESO" title="Usuarios y roles" action={`${roles.length} roles`} /><Notice error={error} success={success} /><p className="note">Los permisos se validan también en la API. Las claves actuales nunca pueden consultarse. Al restablecer una clave se cierran las sesiones y el usuario debe reemplazar la clave temporal al ingresar.</p>{users.length ? <Table headers={["Usuario", "Rol", "Cooperativa", "Seguridad", "Acciones"]} rows={users.map(user => [<div><strong>{user.name}</strong><br /><small>{user.email}</small></div>, <select value={user.role} onChange={event => updateDraft(user.id, { role: event.target.value })}>{roles.map(item => <option key={item.role} value={item.role}>{roleLabels[item.role] ?? item.role}</option>)}</select>, <select value={user.cooperativeId ?? ""} onChange={event => updateDraft(user.id, { cooperativeId: event.target.value || null })}><option value="">Sin cooperativa</option>{cooperatives.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select>, user.mustChangePassword?<Badge value="PENDING" />:<span>Clave personal</span>, <div className="row-actions"><button className="link" disabled={busy === user.id || (user.role === "ANALISTA_COOPERATIVA" && !user.cooperativeId)} onClick={() => save(user)}>{busy === user.id ? "Guardando…" : "Guardar acceso"}</button>{user.id!==currentUserId?<PasswordReset token={token} userId={user.id} userName={user.name} />:<small>Tu cuenta actual</small>}</div>])} /> : <Empty text="No existen usuarios administrativos en la base." />}</section><form className="card form-card" onSubmit={create}><Header eyebrow="NUEVO ACCESO" title="Crear usuario administrativo" /><label>Nombre<input required minLength={3} value={form.fullName} onChange={event => setForm({ ...form, fullName: event.target.value })} /></label><label>Correo<input required type="email" value={form.email} onChange={event => setForm({ ...form, email: event.target.value })} /></label><label>Teléfono<input required minLength={8} value={form.phone} onChange={event => setForm({ ...form, phone: event.target.value })} /></label><label>Clave temporal<input required type="password" minLength={10} value={form.password} onChange={event => setForm({ ...form, password: event.target.value })} /></label><small>10+ caracteres con mayúscula, minúscula, número y símbolo. El usuario deberá reemplazarla al ingresar.</small><label>Rol<select value={form.role} onChange={event => setForm({ ...form, role: event.target.value })}>{roles.filter(item => !["ADMIN", "SUPPORT"].includes(item.role)).map(item => <option key={item.role} value={item.role}>{roleLabels[item.role] ?? item.role}</option>)}</select></label><label>Cooperativa<select value={form.cooperativeId} onChange={event => setForm({ ...form, cooperativeId: event.target.value })}><option value="">Sin cooperativa</option>{cooperatives.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><button className="primary" disabled={busy === "new" || (form.role === "ANALISTA_COOPERATIVA" && !form.cooperativeId)}>{busy === "new" ? "Creando…" : "Crear usuario"}</button></form></div>;
}
