...
function DriverDocuments({ token, driver, admin }: { token: string; driver: any; admin: boolean }) {
  ...
  return <><button className="link" type="button" onClick={load}>Ver documentos</button>{error && <small className="inline-error">{error}</small>}{open && <div className="modal-backdrop" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget) setOpen(false); }}><section className="modal-card document-modal" role="dialog" aria-modal="true"><Header eyebrow="IDENTIDAD" title={driver.name} action={driver.email} />{items.length ? <div className="document-grid">{items.map(item => <article className="document-item" key={item.id}><img src={`data:${item.fileMime};base64,${item.fileBase64}`} alt={labels[item.documentType] ?? item.documentType} /><strong>{labels[item.documentType] ?? item.documentType}</strong><Badge value={item.status} />{item.expiresAt && <small>Vence: {new Date(item.expiresAt).toLocaleDateString()}</small>}{item.reviewNote && <small>{item.reviewNote}</small>}{admin && <div className="row-actions"><button className="link" disabled={busy === item.id} onClick={() => review(item, "ACTIVE")}>Aprobar</button><button className="link danger" disabled={busy === item.id} onClick={() => review(item, "REJECTED")}>Rechazar</button></div>}</article>)}</div> : <Empty text="El conductor todavía no ha cargado documentos." />}<button className="secondary" onClick={() => setOpen(false)}>Cerrar</button></section></div>}</>;
}
