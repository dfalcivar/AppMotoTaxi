...
function Settings({ token, admin }: { token: string; admin: boolean }) {
  ...
  useEffect(() => { Promise.all([apiFetch<{ searchRadiusMeters: number; scheduledTripLeadMinutes: number }>("/v1/admin/settings", token), apiFetch<typeof approvalSettings>("/v1/admin/driver-approval-settings", token)]).then(([settings, approval]) => { setRadius(settings.searchRadiusMeters); setScheduledLead(settings.scheduledTripLeadMinutes ?? 10); setApprovalSettings(approval); }).catch(reason => setError(errorText(reason))); }, [token]);
  async function save(event: React.FormEvent) { event.preventDefault(); setSuccess(""); setError(""); try { const value = await apiFetch<{ searchRadiusMeters: number; scheduledTripLeadMinutes: number }>("/v1/admin/settings", token, { method: "PATCH", body: JSON.stringify({ searchRadiusMeters: radius, scheduledTripLeadMinutes: scheduledLead }) }); setRadius(value.searchRadiusMeters); setScheduledLead(value.scheduledTripLeadMinutes); setSuccess("Configuración operativa guardada."); } catch (reason) { setError(errorText(reason)); } }
