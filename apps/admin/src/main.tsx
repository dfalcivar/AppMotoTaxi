...
function Settings({ token, admin }: { token: string; admin: boolean }) {
  ...
  useEffect(() => { Promise.all([apiFetch<{ searchRadiusMeters: number }>("/v1/admin/settings", token), apiFetch<typeof approvalSettings>("/v1/admin/driver-approval-settings", token)]).then(([settings, approval]) => { setRadius(settings.searchRadiusMeters); setApprovalSettings(approval); }).catch(reason => setError(errorText(reason))); }, [token]);
