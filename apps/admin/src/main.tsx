...
function DriverApprovalInbox({ token, canManageDocuments, onChanged }: { token: string; canManageDocuments: boolean; onChanged: () => void }) {
  ...
  async function decide(driver: any, decision: string) {
    const needsObservation = decision !== "APPROVE";
    const observation = needsObservation ? prompt("Observación obligatoria para el conductor:", driver.approvalObservation ?? "") : "Documentación completa y verificada";
    if (needsObservation && (!observation || observation.trim().length < 5)) return;
    setBusy(driver.id); setError("");
    try { await apiFetch(`/v1/admin/driver-approvals/${driver.id}/decision`, token, { method: "POST", body: JSON.stringify({ decision, observation }) }); await load(); onChanged(); }
...
function Settings({ token, admin }: { token: string; admin: boolean }) {
  ...
  useEffect(() => { Promise.all([apiFetch<{ searchRadiusMeters: number; scheduledTripLeadMinutes: number; scheduledTripMinimumNoticeMinutes: number }>("/v1/admin/settings", token), apiFetch<typeof approvalSettings>("/v1/admin/driver-approval-settings", token)]).then(([settings, approval]) => { setRadius(settings.searchRadiusMeters); setScheduledLead(settings.scheduledTripLeadMinutes ?? 10); setScheduledMinimumNotice(settings.scheduledTripMinimumNoticeMinutes ?? 30); setApprovalSettings(approval); }).catch(reason => setError(errorText(reason))); }, [token]);
  async function save(event: React.FormEvent) { event.preventDefault(); setSuccess(""); setError(""); try { const value = await apiFetch<{ searchRadiusMeters: number; scheduledTripLeadMinutes: number; scheduledTripMinimumNoticeMinutes: number }>("/v1/admin/settings", token, { method: "PATCH", body: JSON.stringify({ searchRadiusMeters: radius, scheduledTripLeadMinutes: scheduledLead, scheduledTripMinimumNoticeMinutes: scheduledMinimumNotice }) }); setRadius(value.searchRadiusMeters); setScheduledLead(value.scheduledTripLeadMinutes); setScheduledMinimumNotice(value.scheduledTripMinimumNoticeMinutes); setSuccess("Configuración operativa guardada."); } catch (reason) { setError(errorText(reason)); } }
