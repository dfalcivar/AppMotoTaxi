...
export async function driverMembershipEligibility(driverId: string, tripId?:string): Promise<DriverEligibility> {
  ...
  const [freeTripBenefit]=await database()`select r.id,r.effective_from as "effectiveFrom",r.effective_until as "effectiveUntil",r.benefit_code as code
    from benefit_redemptions r where r.id=active_free_trip_benefit(${driverId})`;
  if(freeTripBenefit)return {eligible:true,enforcementEnabled:true,benefit:freeTripBenefit};
...
export async function membershipSchedulerTick(): Promise<void> {
  ...
  for (const item of expired) {
    ...
    const [benefitCoverage]=await database()`select coalesce(active_courtesy_benefit(${item.driverId}),active_free_trip_benefit(${item.driverId})) as id`;
  ...
  const due = await database()`select id::text,driver_id::text as "driverId" from driver_memberships where cycle_closed_at is null and suspension_at is not null and suspension_at<=now() and status in ('GRACE_PERIOD','PAYMENT_DUE') and active_courtesy_benefit(driver_id) is null and active_free_trip_benefit(driver_id) is null order by suspension_at limit 100`;
  for (const item of due) {
    const outcome = await database().begin(async tx => {
      ...
      const [coverage]=await tx`select coalesce(active_courtesy_benefit(${item.driverId}),active_free_trip_benefit(${item.driverId})) as id`;if(coverage?.id)return;
      ...
      if (!activeTrip) await tx`update drivers set is_available=false where user_id=${item.driverId} and active_courtesy_benefit(user_id) is null and active_free_trip_benefit(user_id) is null`;
      return status;
    });
  ...
  const pendingAfterTrip = await database()`
    ...
    where dm.cycle_closed_at is null and dm.status='SUSPENSION_PENDING_ACTIVE_TRIP' and active_courtesy_benefit(dm.driver_id) is null and active_free_trip_benefit(dm.driver_id) is null
  ...
  for (const item of pendingAfterTrip) {
    ...
    if (updated) {
      await database()`update drivers set is_available=false where user_id=${item.driverId} and active_courtesy_benefit(user_id) is null and active_free_trip_benefit(user_id) is null`;
