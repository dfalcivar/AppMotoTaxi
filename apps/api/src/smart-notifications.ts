...
async function processScheduledCampaigns(){
  ...
  for(const item of campaigns){
    try{
      ...
      const segment=segmentSchema.parse(campaign.segment??{}),users=await candidatesForSegment(segment);
...
export async function registerSmartNotificationRoutes(app:FastifyInstance){
  ...
  app.get('/v1/admin/notifications/campaigns',async(request,reply)=>{try{requirePermission(request,'notification_campaigns:view');return database()`select c.id::text,c.name,c.campaign_type as type,c.title,c.segment,c.status,c.scheduled_at as "scheduledAt",c.created_at as "createdAt",count(r.id)::int recipients,count(r.id) filter(where r.status='SENT')::int sent,count(r.id) filter(where r.status='OPENED')::int opened from notification_campaigns c left join notification_campaign_recipients r on r.campaign_id=c.id group by c.id order by c.created_at desc limit 200`;}catch(error){return guard(error,reply);}});
