...
import {notificationPreferenceAllows,updateSmartSaturation} from './notification-preferences.js';

const modes=['OFF','TEST','ON'] as const;
const patternTypes=['FREQUENT_TRIP','RETURN_HOME','FAVORITE_DESTINATION','REACTIVATION'] as const;
const campaignTypes=['CAMPAIGN','EVENT','PROMOTIONAL','APP_UPDATE'] as const;
const appUpdateAudienceModes=['TESTERS','OUTDATED','MANUAL'] as const;
const configSchema=z.object({
  ...
  maxPerUserPerWeek:z.number().int().min(0).max(50),ignoredToReduceFrequency:z.number().int().min(1).max(20),ignoredToPause:z.number().int().min(2).max(50),autoPauseDays:z.number().int().min(1).max(90),reducedFrequencyMultiplier:z.number().int().min(1).max(10),
...
const segmentSchema=z.object({zones:z.array(z.string().min(1).max(100)).max(50).default([]),roles:z.array(z.enum(['PASSENGER','DRIVER'])).max(2).default([]),platforms:z.array(z.enum(['ANDROID','IOS'])).max(2).default([]),activeWithinDays:z.number().int().min(1).max(365).optional(),userIds:z.array(z.string().uuid()).max(500).default([]),userEmails:z.array(z.string().trim().email().transform(value=>value.toLowerCase())).max(500).default([])});
const appUpdateMetadataSchema=z.object({publishedVersion:z.string().trim().regex(/^\d+(?:\.\d+){1,3}(?:[-+][0-9A-Za-z.-]+)?$/).max(50),publishedBuild:z.number().int().min(1),targetPlatform:z.enum(['ANDROID','IOS']),audienceMode:z.enum(appUpdateAudienceModes),storeUrl:z.string().url().max(500),updateAction:z.enum(['OPEN_STORE','UPDATE_NOW']).default('OPEN_STORE')});
const reminderConfigSchema=z.object({passengerScheduledReminderMinutes:z.array(z.number().int().min(1).max(1440)).max(10),passengerScheduledMaxReminders:z.number().int().min(0).max(10),allowedStartTime:z.string().regex(/^([01]\d|2[0-3]):[0-5]\d$/),allowedEndTime:z.string().regex(/^([01]\d|2[0-3]):[0-5]\d$/),driverMembershipReminderDays:z.array(z.number().int().min(1).max(90)).max(10),driverMembershipMaxReminders:z.number().int().min(0).max(10),driverDocumentReminderDays:z.array(z.number().int().min(1).max(365)).max(10)}).refine(value=>value.allowedStartTime<value.allowedEndTime,{message:'INVALID_ALLOWED_TIME'});
const campaignSchema=z.object({name:z.string().trim().min(3).max(120),campaignType:z.enum(campaignTypes),title:z.string().trim().min(1).max(120),body:z.string().trim().min(1).max(500),deepLink:z.string().trim().max(500).optional().or(z.literal('')),action:z.string().trim().max(80).optional().or(z.literal('')),metadata:z.record(z.string(),z.unknown()).default({}),segment:segmentSchema.default({zones:[],roles:[],platforms:[],userIds:[],userEmails:[]})}).superRefine((value,context)=>{if(value.campaignType!=='APP_UPDATE')return;const parsed=appUpdateMetadataSchema.safeParse(value.metadata);if(!parsed.success)for(const issue of parsed.error.issues)context.addIssue({...issue,path:['metadata',...issue.path]});if(parsed.success&&value.deepLink!==parsed.data.storeUrl)context.addIssue({code:'custom',path:['deepLink'],message:'APP_UPDATE_STORE_URL_MISMATCH'});if(parsed.success&&value.action!==parsed.data.updateAction)context.addIssue({code:'custom',path:['action'],message:'APP_UPDATE_ACTION_MISMATCH'});if(parsed.success&&value.segment.platforms.length>0&&!value.segment.platforms.includes(parsed.data.targetPlatform))context.addIssue({code:'custom',path:['segment','platforms'],message:'APP_UPDATE_PLATFORM_MISMATCH'});if(parsed.success&&parsed.data.audienceMode==='MANUAL'&&value.segment.userIds.length+value.segment.userEmails.length===0)context.addIssue({code:'custom',path:['segment','userEmails'],message:'APP_UPDATE_MANUAL_AUDIENCE_REQUIRED'});});
...
export function compareAppVersions(left:unknown,right:unknown):number{const parts=(value:unknown)=>(String(value??'').split(/[+-]/,1).at(0)??'').split('.').map(item=>Number.parseInt(item,10)||0);const a=parts(left),b=parts(right);for(let index=0;index<Math.max(a.length,b.length);index++){const difference=(a[index]??0)-(b[index]??0);if(difference)return difference>0?1:-1;}return 0;}
function appUpdateDisposition(row:any,target:z.infer<typeof appUpdateMetadataSchema>){if(row.appBuild!=null)return number(row.appBuild)>=target.publishedBuild?'UPDATED':'OUTDATED';if(row.appVersion)return compareAppVersions(row.appVersion,target.publishedVersion)>=0?'UPDATED':'OUTDATED';return 'UNKNOWN';}
...
async function eligibility(patternId:string,providedConfig?:Config):Promise<any>{
  ...
  const [row]=await database()`select p.id::text,p.user_id::text as "userId",p.pattern_type as type,p.origin_reference as origin,p.destination_reference as destination,p.destination_lat as "destinationLat",p.destination_lng as "destinationLng",p.week_days as "weekDays",p.average_time::text as "averageTime",p.matches_count as matches,p.trips_analyzed as "tripsAnalyzed",p.confidence_score::float8 as confidence,u.full_name as user,
    ...
    (select count(*)::int from user_notifications n where n.user_id=p.user_id and n.category='SMART' and n.created_at>=now()-interval '7 days') as "sentThisWeek",
  ...
  const saturation=await updateSmartSaturation(String(row.userId),config);
  if(saturation.state==='USER_DISABLED')reasons.push('USER_DISABLED');
  if(saturation.state==='AUTO_PAUSED')reasons.push('AUTO_PAUSED');
  ...
  if(number(row.sentThisWeek)>=config.maxPerUserPerWeek)reasons.push('WEEKLY_LIMIT_REACHED');
  const effectiveInterval=config.minimumIntervalMinutes*(saturation.reduced?config.reducedFrequencyMultiplier:1);
  if(row.lastSmartAt&&Date.now()-new Date(row.lastSmartAt).getTime()<effectiveInterval*60000)reasons.push(saturation.reduced?'REDUCED_FREQUENCY_INTERVAL':'MINIMUM_INTERVAL_NOT_REACHED');
...
async function candidatesForSegment(segment:z.infer<typeof segmentSchema>){
  return database()`select distinct u.id::text,u.full_name as name,u.email,u.role::text as role,d.platform,d.last_seen_at as "lastSeenAt",d.app_version as "appVersion",d.app_build as "appBuild",area.name as zone,
    exists(select 1 from notification_test_users x where x.user_id=u.id and x.enabled) as tester
    ...
    where u.status='ACTIVE' and u.deleted_at is null
      and (${segment.userIds.length+segment.userEmails.length}=0 or u.id=any(${segment.userIds}::uuid[]) or lower(u.email)=any(${segment.userEmails}::text[]))
...
async function candidatesForCampaign(campaign:any,segment:z.infer<typeof segmentSchema>){
  const rows=await candidatesForSegment(segment);
  if(campaign.campaignType==='PROMOTIONAL'){
    const allowed=[] as any[];
    for(const row of rows)if(await notificationPreferenceAllows(String(row.id),'PROMOTIONAL_NOTIFICATIONS'))allowed.push(row);
    return allowed;
  }
  if(campaign.campaignType!=='APP_UPDATE')return rows;
  const target=appUpdateMetadataSchema.parse(storedJson(campaign.metadata??{}));
  return rows.filter((row:any)=>row.platform===target.targetPlatform)
    .filter((row:any)=>target.audienceMode!=='TESTERS'||row.tester)
    .filter((row:any)=>target.audienceMode!=='OUTDATED'||appUpdateDisposition(row,target)==='OUTDATED');
}

async function estimateSegment(segment:z.infer<typeof segmentSchema>,promotional=false){
  const rows=await database()`select distinct u.id::text,u.full_name as name,u.email,d.platform,d.last_seen_at as "lastSeenAt",d.enabled,d.invalidated_at as "invalidatedAt",
    (d.token is not null and d.enabled and d.invalidated_at is null and d.last_seen_at>now()-interval '90 days') as "validToken"
    from users u
    left join lateral(select token,platform,last_seen_at,enabled,invalidated_at from device_tokens where user_id=u.id order by (enabled and invalidated_at is null and last_seen_at>now()-interval '90 days') desc,last_seen_at desc limit 1)d on true
    ...
    order by u.id::text`;
  const preferences=promotional?new Map((await database()`select user_id::text as id,enabled,state from user_notification_preferences where context='COMMON' and preference_key='PROMOTIONAL_NOTIFICATIONS'`).map((row:any)=>[String(row.id),row])):new Map();
  const users=rows.map((row:any)=>{const preference:any=preferences.get(String(row.id));const preferenceAllowed=!preference||(preference.enabled&&preference.state==='ENABLED');return {...row,preferenceAllowed,reason:!preferenceAllowed?'PREFERENCE_DISABLED':row.validToken?'ELIGIBLE':!row.lastSeenAt?'NO_TOKEN':!row.enabled?'TOKEN_DISABLED':row.invalidatedAt?'TOKEN_INVALIDATED':'TOKEN_STALE'};});
  const valid=users.filter((row:any)=>row.validToken&&row.preferenceAllowed);
  return {usersFound:rows.length,total:valid.length,android:valid.filter((row:any)=>row.platform==='ANDROID').length,ios:valid.filter((row:any)=>row.platform==='IOS').length,withoutValidToken:users.filter((row:any)=>!row.validToken).length,preferenceDisabled:users.filter((row:any)=>!row.preferenceAllowed).length,excluded:rows.length-valid.length,users};
}

async function estimateAppUpdate(segment:z.infer<typeof segmentSchema>,rawMetadata:unknown){
  const target=appUpdateMetadataSchema.parse(storedJson(rawMetadata??{}));
  const rows=await database()`select distinct u.id::text,u.full_name as name,u.email,d.platform,d.last_seen_at as "lastSeenAt",d.enabled,d.invalidated_at as "invalidatedAt",d.app_version as "appVersion",d.app_build as "appBuild",
    exists(select 1 from notification_test_users x where x.user_id=u.id and x.enabled) as tester,
    (d.token is not null and d.enabled and d.invalidated_at is null and d.last_seen_at>now()-interval '90 days') as "validToken"
    from users u
    left join lateral(select token,platform,last_seen_at,enabled,invalidated_at,app_version,app_build from device_tokens where user_id=u.id order by (enabled and invalidated_at is null and last_seen_at>now()-interval '90 days') desc,last_seen_at desc limit 1)d on true
    left join lateral(select t.service_area_id,t.requested_at from trips t where t.passenger_id=u.id or t.driver_id=u.id order by t.requested_at desc limit 1) recent on true
    left join service_areas area on area.id=recent.service_area_id
    where u.status='ACTIVE' and u.deleted_at is null
      and (${target.audienceMode!=='TESTERS'} or exists(select 1 from notification_test_users x where x.user_id=u.id and x.enabled))
      and (${target.audienceMode!=='MANUAL'} or u.id=any(${segment.userIds}::uuid[]) or lower(u.email)=any(${segment.userEmails}::text[]))
      and (d.platform=${target.targetPlatform} or d.platform is null)
      and (${segment.roles.length}=0 or u.role::text=any(${segment.roles}::text[]) or exists(select 1 from mobile_account_roles r where r.user_id=u.id and r.role=any(${segment.roles}::text[])))
      and (${segment.zones.length}=0 or area.name=any(${segment.zones}::text[]) or recent.service_area_id::text=any(${segment.zones}::text[]))
      and (${segment.activeWithinDays??null}::int is null or coalesce(recent.requested_at,u.updated_at,u.created_at)>=now()-(coalesce(${segment.activeWithinDays??null},365)*interval '1 day'))
    order by u.id::text`;
  const users=rows.map((row:any)=>{const versionStatus=appUpdateDisposition(row,target);const versionEligible=target.audienceMode!=='OUTDATED'||versionStatus==='OUTDATED';const reason=!row.validToken?(!row.lastSeenAt?'NO_TOKEN':!row.enabled?'TOKEN_DISABLED':row.invalidatedAt?'TOKEN_INVALIDATED':'TOKEN_STALE'):target.audienceMode==='OUTDATED'&&versionStatus==='UNKNOWN'?'VERSION_UNKNOWN':!versionEligible?'ALREADY_UPDATED':'ELIGIBLE';return {...row,versionStatus,reason,eligible:Boolean(row.validToken&&versionEligible)};});
  const eligible=users.filter((row:any)=>row.eligible);
  return {testersFound:users.filter((row:any)=>row.tester).length,usersFound:users.length,alreadyUpdated:users.filter((row:any)=>row.versionStatus==='UPDATED').length,outdated:users.filter((row:any)=>row.versionStatus==='OUTDATED').length,withoutVersion:users.filter((row:any)=>row.versionStatus==='UNKNOWN').length,withoutValidToken:users.filter((row:any)=>!row.validToken).length,total:eligible.length,android:eligible.filter((row:any)=>row.platform==='ANDROID').length,ios:eligible.filter((row:any)=>row.platform==='IOS').length,excluded:users.length-eligible.length,users};
}
...
async function processScheduledCampaigns(){
  ...
  for(const item of campaigns){
    try{
      ...
      const segment=segmentSchema.parse(storedJson(campaign.segment??{})),users=await candidatesForCampaign(campaign,segment);
...
export async function registerSmartNotificationRoutes(app:FastifyInstance){
  ...
  app.get('/v1/admin/notifications/reminder-config',async(request,reply)=>{try{requirePermission(request,'notifications:view');const [row]=await database()`select passenger_scheduled_reminder_minutes as "passengerScheduledReminderMinutes",passenger_scheduled_max_reminders as "passengerScheduledMaxReminders",reminder_allowed_start_time::text as "allowedStartTime",reminder_allowed_end_time::text as "allowedEndTime",driver_membership_reminder_days as "driverMembershipReminderDays",driver_membership_max_reminders as "driverMembershipMaxReminders",driver_document_reminder_days as "driverDocumentReminderDays" from operational_settings where id=1`;return {...row,allowedStartTime:time(row?.allowedStartTime),allowedEndTime:time(row?.allowedEndTime)};}catch(error){return guard(error,reply);}});
  app.put('/v1/admin/notifications/reminder-config',async(request,reply)=>{try{const actor=requirePermission(request,'notifications:manage');const value=reminderConfigSchema.parse(request.body);await database()`update operational_settings set passenger_scheduled_reminder_minutes=${value.passengerScheduledReminderMinutes},passenger_scheduled_max_reminders=${value.passengerScheduledMaxReminders},reminder_allowed_start_time=${value.allowedStartTime}::time,reminder_allowed_end_time=${value.allowedEndTime}::time,driver_membership_reminder_days=${value.driverMembershipReminderDays},driver_membership_max_reminders=${value.driverMembershipMaxReminders},driver_document_reminder_days=${value.driverDocumentReminderDays},updated_by=${actor.id!},updated_at=now() where id=1`;await database()`insert into audit_log(actor_id,action,entity_type,entity_id,next_value,reason) values(${actor.id!},'NOTIFICATION_REMINDER_CONFIG_UPDATED','OPERATIONAL_SETTINGS','1',${JSON.stringify(value)}::jsonb,'Parámetros de recordatorios actualizados')`;return value;}catch(error){return guard(error,reply);}});
  app.put('/v1/admin/notifications/smart/config',async(request,reply)=>{try{const actor=requirePermission(request,'notifications:manage');const value=configSchema.parse(request.body);const [previous]=await database()`select to_jsonb(smart_notification_config) value from smart_notification_config where id=1`;const [row]=await database()`update smart_notification_config set mode=${value.mode},frequent_trip_enabled=${value.frequentTripEnabled},return_trip_enabled=${value.returnTripEnabled},favorite_destination_enabled=${value.favoriteDestinationEnabled},reactivation_enabled=${value.reactivationEnabled},minimum_trips=${value.minimumTrips},minimum_matches=${value.minimumMatches},minimum_confidence=${value.minimumConfidence},analysis_window_days=${value.analysisWindowDays},schedule_tolerance_minutes=${value.scheduleToleranceMinutes},notification_lead_minutes=${value.notificationLeadMinutes},max_per_user_per_day=${value.maxPerUserPerDay},max_per_user_per_week=${value.maxPerUserPerWeek},ignored_to_reduce_frequency=${value.ignoredToReduceFrequency},ignored_to_pause=${value.ignoredToPause},auto_pause_days=${value.autoPauseDays},reduced_frequency_multiplier=${value.reducedFrequencyMultiplier},minimum_interval_minutes=${value.minimumIntervalMinutes},allowed_start_time=${value.allowedStartTime}::time,allowed_end_time=${value.allowedEndTime}::time,enabled_week_days=${value.enabledWeekDays},inactive_user_days=${value.inactiveUserDays},scheduler_interval_minutes=${value.schedulerIntervalMinutes},updated_by=${actor.id!},updated_at=now() where id=1 returning updated_at as "updatedAt"`;if(!row)throw new Error('SMART_CONFIG_NOT_FOUND');await database()`insert into audit_log(actor_id,action,entity_type,entity_id,previous_value,next_value,reason) values (${actor.id!},'SMART_NOTIFICATION_CONFIG_UPDATED','SMART_NOTIFICATION_CONFIG','1',${JSON.stringify(previous?.value??{})}::jsonb,${JSON.stringify(value)}::jsonb,'Configuración modificada desde Notificaciones Inteligentes')`;return {...value,updatedAt:row.updatedAt};}catch(error){return guard(error,reply);}});
  app.get('/v1/admin/notifications/app-versions',async(request,reply)=>{try{requirePermission(request,'notifications:view');return database()`select platform,latest_version as "latestVersion",latest_build as "latestBuild",minimum_supported_version as "minimumSupportedVersion",minimum_supported_build as "minimumSupportedBuild",update_policy as "updatePolicy",required_update_enabled as "requiredUpdateEnabled",message,store_url as "storeUrl",updated_at as "updatedAt" from app_version_config order by platform`;}catch(error){return guard(error,reply);}});
  app.put('/v1/admin/notifications/app-versions/:platform',async(request,reply)=>{try{const actor=requirePermission(request,'notifications:manage');const platform=z.enum(['ANDROID','IOS']).parse((request.params as any).platform);const version=z.string().trim().regex(/^\d+(?:\.\d+){1,3}(?:[-+][0-9A-Za-z.-]+)?$/).max(50).nullable();const body=z.object({latestVersion:version,latestBuild:z.number().int().min(1).nullable(),minimumSupportedVersion:version,minimumSupportedBuild:z.number().int().min(1).nullable(),updatePolicy:z.enum(['RECOMMENDED','REQUIRED']).default('RECOMMENDED'),requiredUpdateEnabled:z.literal(false).default(false),message:z.string().trim().min(1).max(500),storeUrl:z.string().url().max(500).nullable()}).parse(request.body);const [row]=await database()`update app_version_config set latest_version=${body.latestVersion},latest_build=${body.latestBuild},minimum_supported_version=${body.minimumSupportedVersion},minimum_supported_build=${body.minimumSupportedBuild},update_policy=${body.updatePolicy},required_update_enabled=false,message=${body.message},store_url=${body.storeUrl},updated_by=${actor.id!},updated_at=now() where platform=${platform} returning platform,latest_version as "latestVersion",latest_build as "latestBuild",minimum_supported_version as "minimumSupportedVersion",minimum_supported_build as "minimumSupportedBuild",update_policy as "updatePolicy",required_update_enabled as "requiredUpdateEnabled",message,store_url as "storeUrl",updated_at as "updatedAt"`;return row;}catch(error){return guard(error,reply);}});
  ...
  app.get('/v1/admin/notifications/campaigns',async(request,reply)=>{try{requirePermission(request,'notification_campaigns:view');const rows=await database()`select c.id::text,c.name,c.campaign_type as type,c.title,c.segment,c.status,c.scheduled_at as "scheduledAt",c.created_at as "createdAt",count(distinct r.id)::int recipients,count(distinct r.id) filter(where r.status in ('SENT','OPENED'))::int sent,count(distinct r.id) filter(where r.status='OPENED')::int opened,count(distinct e.notification_id) filter(where e.event='APP_UPDATED')::int updated from notification_campaigns c left join notification_campaign_recipients r on r.campaign_id=c.id left join notification_analytics_events e on e.notification_id=r.notification_id group by c.id order by c.created_at desc limit 200`;return rows.map(row=>({...row,segment:storedJson(row.segment)}));}catch(error){return guard(error,reply);}});
  ...
  app.get('/v1/admin/notifications/campaigns/:id',async(request,reply)=>{try{requirePermission(request,'notification_campaigns:view');const id=z.string().uuid().parse((request.params as any).id);const [row]=await database()`select id::text,name,campaign_type as "campaignType",title,body,deep_link as "deepLink",action,metadata,segment,status,scheduled_at as "scheduledAt",created_at as "createdAt",updated_at as "updatedAt" from notification_campaigns where id=${id}`;if(!row)return reply.code(404).send({error:'CAMPAIGN_NOT_FOUND'});const recipients=await database()`select u.full_name as name,u.email,r.status,r.error_code as "errorCode",r.error_message as "errorMessage",r.sent_at as "sentAt",exists(select 1 from notification_analytics_events e where e.notification_id=r.notification_id and e.event='STORE_OPENED') as "storeOpened",exists(select 1 from notification_analytics_events e where e.notification_id=r.notification_id and e.event='APP_UPDATED') as updated from notification_campaign_recipients r join users u on u.id=r.user_id where r.campaign_id=${id} order by u.email`;return {...row,metadata:storedJson(row.metadata),segment:storedJson(row.segment),recipients,updateMetrics:{notified:recipients.filter((item:any)=>['SENT','OPENED'].includes(item.status)).length,storeOpened:recipients.filter((item:any)=>item.storeOpened).length,updated:recipients.filter((item:any)=>item.updated).length,pending:recipients.filter((item:any)=>['SENT','OPENED'].includes(item.status)&&!item.updated).length}};}catch(error){return guard(error,reply);}});
  ...
  app.post('/v1/admin/notifications/campaigns/:id/estimate',async(request,reply)=>{try{requirePermission(request,'notification_campaigns:view');const id=z.string().uuid().parse((request.params as any).id);const [campaign]=await database()`select campaign_type as "campaignType",segment,metadata from notification_campaigns where id=${id}`;if(!campaign)return reply.code(404).send({error:'CAMPAIGN_NOT_FOUND'});const segment=segmentSchema.parse(storedJson(campaign.segment));return campaign.campaignType==='APP_UPDATE'?estimateAppUpdate(segment,campaign.metadata):estimateSegment(segment,campaign.campaignType==='PROMOTIONAL');}catch(error){return guard(error,reply);}});
