...
async function estimateSegment(segment:z.infer<typeof segmentSchema>){
  ...
  const users=rows.map((row:any)=>({...row,reason:row.validToken?'ELIGIBLE':!row.lastSeenAt?'NO_TOKEN':!row.enabled?'TOKEN_DISABLED':row.invalidatedAt?'TOKEN_INVALIDATED':'TOKEN_STALE'}));
  return {usersFound:rows.length,total:valid.length,android:valid.filter((row:any)=>row.platform==='ANDROID').length,ios:valid.filter((row:any)=>row.platform==='IOS').length,withoutValidToken:rows.length-valid.length,excluded:rows.length-valid.length,users};
}
