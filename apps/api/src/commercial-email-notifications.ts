...
export async function registerCommercialEmailNotificationRoutes(app:FastifyInstance){
  ...
  app.patch('/v1/admin/notifications/commercial-email/events/:event',async(request,reply)=>{try{const actor=requirePermission(request,'notifications:manage'),event=eventSchema.parse((request.params as any).event),body=z.object({enabled:z.boolean(),scheduleConfig:z.unknown().optional()}).parse(request.body),[previous]=await database()`select enabled,schedule_config as "scheduleConfig" from notification_event_definitions where event_type=${event}`;if(!previous)return reply.code(404).send({error:'NOTIFICATION_EVENT_NOT_FOUND'});const scheduleConfig=body.scheduleConfig===undefined?previous.scheduleConfig:z.object({daysBefore:z.coerce.number().int().min(1).max(90).optional(),dayOfMonth:z.coerce.number().int().min(1).max(28).optional()}).parse(body.scheduleConfig);const next={enabled:body.enabled,scheduleConfig};const [row]=await database()`update notification_event_definitions set enabled=${body.enabled},schedule_config=${JSON.stringify(scheduleConfig)}::jsonb,updated_by=${actor.id!},updated_at=now() where event_type=${event} returning event_type as "eventType",enabled,schedule_config as "scheduleConfig"`;await database()`insert into audit_log(actor_id,action,entity_type,entity_id,previous_value,next_value,reason) values(${actor.id!},'COMMERCIAL_EMAIL_EVENT_UPDATED','NOTIFICATION_EVENT',${event},${JSON.stringify(previous)}::jsonb,${JSON.stringify(next)}::jsonb,'Configuración multicanal comercial actualizada')`;return row;}catch(error){return guard(error,reply);}});
