...
import { commercialEmailSchedulerTick } from './commercial-email-notifications.js';
...
const commercialSettingsSchema=z.object({
  ...
  maxActivePerZone:z.number().int().min(1).max(100),commercialEmails:z.array(z.string().email()).max(20),renewalContactUrl:z.string().url().max(500).nullable().default(null)
});
...
export async function advertisingSchedulerTick(): Promise<void> {
  ...
  await commercialEmailSchedulerTick();
}

export async function registerCommercialRoutes(app: FastifyInstance) {
  app.get("/v1/admin/commercial/campaigns/:id/communications",async(request,reply)=>{try{const actor=requirePermission(request,"commercial:campaigns:view"),own=actor.role==="COMMERCIAL",id=z.string().uuid().parse((request.params as any).id);return database()`select d.id::text,d.event_type as "eventType",d.recipient_email as recipient,d.subject,d.status,d.attempts,d.scheduled_at as "scheduledAt",d.sent_at as "sentAt",d.last_error_code as "errorCode",d.last_error_message as "errorMessage",d.is_test as "isTest" from notification_email_deliveries d join affiliate_banners b on b.id=d.campaign_id join advertising_orders o on o.id=b.order_id where d.campaign_id=${id} and (not ${own} or o.assigned_commercial_id=${actor.id}) order by d.created_at desc`;}catch(error){return adminError(error,reply);}});
  ...
  app.get("/v1/admin/commercial/settings",async(request,reply)=>{try{requirePermission(request,"commercial:plans:manage");const [settings]=await database()`select advertising_invitation_days as "invitationDays",advertising_max_image_bytes as "maxImageBytes",advertising_banner_width as "bannerWidth",advertising_banner_height as "bannerHeight",advertising_max_active_per_zone as "maxActivePerZone",advertising_commercial_emails as "commercialEmails",advertising_renewal_contact_url as "renewalContactUrl" from operational_settings where id=1`;const methods=await database()`select id::text,code,name,instructions,active,requires_proof as "requiresProof",sort_order as "sortOrder" from advertising_payment_methods order by sort_order,name`;return {...settings,paymentMethods:methods};}catch(error){return adminError(error,reply);} });
  app.patch("/v1/admin/commercial/settings",async(request,reply)=>{try{const actor=requirePermission(request,"commercial:plans:manage"),body=commercialSettingsSchema.parse(request.body);const [settings]=await database()`update operational_settings set advertising_invitation_days=${body.invitationDays},advertising_max_image_bytes=${body.maxImageBytes},advertising_banner_width=${body.bannerWidth},advertising_banner_height=${body.bannerHeight},advertising_max_active_per_zone=${body.maxActivePerZone},advertising_commercial_emails=${body.commercialEmails},advertising_renewal_contact_url=${body.renewalContactUrl},updated_at=now(),updated_by=${actor.id} where id=1 returning advertising_invitation_days as "invitationDays",advertising_max_image_bytes as "maxImageBytes",advertising_banner_width as "bannerWidth",advertising_banner_height as "bannerHeight",advertising_max_active_per_zone as "maxActivePerZone",advertising_commercial_emails as "commercialEmails",advertising_renewal_contact_url as "renewalContactUrl"`;await persistAudit(actor,"ADVERTISING_SETTINGS_UPDATED","OPERATIONAL_SETTINGS","1","Configuración comercial actualizada");return settings;}catch(error){return adminError(error,reply);} });
