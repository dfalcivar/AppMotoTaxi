...
async function createInvitation(leadId: string, source: string, actorId?: string, purpose = "APPLICATION", correctionFields: string[] = []) {
  ...
  const [invitation] = await database()`insert into advertising_invitations(lead_id,advertiser_id,token_hash,status,source,purpose,correction_fields,created_by,expires_at)
    select lead.id,lead.advertiser_id,${tokenHash(token)},${purpose === "CORRECTION" ? "CORRECTION" : "CREATED"},${source},${purpose},${correctionFields},${actorId ?? null},now()+(${days}*interval '1 day')
...
export async function advertisingSchedulerTick(): Promise<void> {
  ...
  await database()`with changed as (update affiliate_banners set campaign_status='EXPIRED',active=false,updated_at=now() where campaign_status in ('ACTIVE','SCHEDULED') and ends_at is not null and ends_at<=now() returning id,campaign_status) insert into campaign_status_history(campaign_id,from_status,to_status,note) select id,campaign_status,'EXPIRED','Finalización automática' from changed`;
...
export async function registerCommercialRoutes(app: FastifyInstance) {
  ...
  app.get("/v1/public/advertising/invitations/:token", async (request, reply) => { try {
    ...
    return { status: invitation.status === "CREATED" ? "OPENED" : invitation.status, purpose: invitation.purpose, correctionFields: invitation.correction_fields, expiresAt: invitation.expires_at, lead: { code: invitation.lead_code, businessName: invitation.business_name, contactName: invitation.contact_name, phone: invitation.phone_e164, email: invitation.email, city: invitation.city, businessType: invitation.business_type, interest: invitation.interest }, draft: invitation.draft_data };
  } catch (error) { return publicError(error, reply); } });
  ...
  app.post("/v1/public/advertising/invitations/:token/submit", async (request, reply) => { try {
    ...
    const result = await database().begin(async tx => {
      ...
      let paymentStatus = "PENDING"; let proof: Buffer | null = null;
      ...
      await tx`update advertising_invitations set status='SUBMITTED',submitted_at=now(),advertiser_id=${advertiser.id},draft_data=${JSON.stringify(body)}::jsonb,updated_at=now() where id=${invitation.id}`;
  ...
  app.get("/v1/admin/commercial/dashboard", async (request, reply) => { try { const actor = requirePermission(request, "commercial:dashboard"), actorId=actor.id ?? null, own = actor.role === "COMMERCIAL";
    const [row] = await database()`select
      (select count(*)::int from advertising_leads l where l.status='NEW' and (not ${own} or l.assigned_commercial_id=${actorId})) as "newLeads",
      (select count(*)::int from advertising_leads l where l.status='REQUIRES_CONTACT' and (not ${own} or l.assigned_commercial_id=${actorId})) as "requiresContact",
  ...
  app.get("/v1/admin/commercial/leads", async (request, reply) => { try { const actor=requirePermission(request,"commercial:leads:view"),q=listSchema.parse(request.query),own=actor.role==="COMMERCIAL";
    return database()`select l.id::text,l.code,l.business_name as "businessName",l.contact_name as "contactName",l.phone_e164 as phone,l.email,l.city,l.business_type as "businessType",l.interest,l.source,l.status,l.assigned_commercial_id::text as "assignedCommercialId",u.full_name as "assignedCommercial",l.created_at as "createdAt",l.updated_at as "updatedAt" from advertising_leads l left join users u on u.id=l.assigned_commercial_id where (not ${own} or l.assigned_commercial_id=${actor.id} or l.created_by=${actor.id}) and (${q.status ?? null}::text is null or l.status=${q.status ?? null}) and (${q.source ?? null}::text is null or l.source=${q.source ?? null}) and (${q.search ?? null}::text is null or l.business_name ilike ${q.search ? `%${q.search}%` : null} or l.contact_name ilike ${q.search ? `%${q.search}%` : null} or l.email ilike ${q.search ? `%${q.search}%` : null}) order by l.created_at desc limit ${q.limit} offset ${q.offset}`;
  }catch(error){return adminError(error,reply);} });
  app.patch("/v1/admin/commercial/leads/:id",async(request,reply)=>{try{const actor=requirePermission(request,"commercial:leads:manage"),body=leadUpdateSchema.parse(request.body),id=z.string().uuid().parse((request.params as any).id);const [item]=await database()`update advertising_leads set status=coalesce(${body.status??null},status),assigned_commercial_id=case when ${body.assignedCommercialId===undefined} then assigned_commercial_id else ${body.assignedCommercialId??null}::uuid end,last_contact_at=case when ${body.note??null}::text is not null then now() else last_contact_at end,updated_at=now() where id=${id} and (${actor.role!=="COMMERCIAL"} or assigned_commercial_id=${actor.id} or created_by=${actor.id}) returning id::text,code,status`;if(!item)throw new Error("LEAD_NOT_FOUND");await persistAudit(actor,"ADVERTISING_LEAD_UPDATED","ADVERTISING_LEAD",id,body.note||body.status||"Actualización comercial");return item;}catch(error){return adminError(error,reply);} });
  app.post("/v1/admin/commercial/invitations",async(request,reply)=>{try{const actor=requirePermission(request,"commercial:leads:manage"),body=inviteSchema.parse(request.body);const invitation=await createInvitation(body.leadId,"COMMERCIAL",actor.id,body.purpose,body.correctionFields);const [lead]=await database()`select contact_name,email,business_name from advertising_leads where id=${body.leadId}`;if(!lead)throw new Error("LEAD_NOT_FOUND");void sendTransactionalEmail({to:String(lead.email),subject:`Invitación comercial Costa-Go`,text:`Continúa la solicitud de ${lead.business_name}: ${invitation.url}`,html:`<p>Hola ${lead.contact_name}.</p><p><a href="${invitation.url}">Continuar solicitud comercial</a></p>`}).catch(()=>false);await persistAudit(actor,"ADVERTISING_INVITATION_CREATED","ADVERTISING_INVITATION",String(invitation.id),body.purpose);return reply.code(201).send(invitation);}catch(error){return adminError(error,reply);} });
  ...
  app.get("/v1/admin/commercial/payments/:id/proof",async(request,reply)=>{try{requirePermission(request,"commercial:payments:view");const id=z.string().uuid().parse((request.params as any).id);const [file]=await database()`select proof_mime,proof_data from advertising_payments where id=${id}`;if(!file?.proof_data)throw new Error("PAYMENT_PROOF_NOT_FOUND");return reply.header("content-type",String(file.proof_mime)).header("cache-control","private, no-store").send(file.proof_data);}catch(error){return adminError(error,reply);} });
  app.post("/v1/admin/commercial/payments/:id/review",async(request,reply)=>{try{const actor=requirePermission(request,"commercial:payments:review"),id=z.string().uuid().parse((request.params as any).id),body=paymentReviewSchema.parse(request.body);if(body.decision==="REJECT"&&body.reason.length<5)return reply.code(400).send({error:"REASON_REQUIRED"});const result=await database().begin(async tx=>{const [pay]=await tx`select pay.*,o.id as order_id from advertising_payments pay join advertising_orders o on o.id=pay.order_id where pay.id=${id} for update`;if(!pay)throw new Error("PAYMENT_NOT_FOUND");const status=body.decision==="APPROVE"?"APPROVED":body.decision==="REJECT"?"REJECTED":"REFUNDED";await tx`update advertising_payments set status=${status},reviewed_by=${actor.id},reviewed_at=now(),rejection_reason=${body.reason||null},updated_at=now() where id=${id}`;const orderStatus=status==="APPROVED"?"PAID":status==="REFUNDED"?"REFUNDED":"PENDING_PAYMENT";await tx`update advertising_orders set status=${orderStatus},updated_at=now() where id=${pay.order_id}`;const [campaign]=await tx`update affiliate_banners set campaign_status=${status==="APPROVED"?"PENDING_REVIEW":"PENDING_PAYMENT"},review_requested_at=case when ${status}='APPROVED' then now() else review_requested_at end,updated_at=now() where order_id=${pay.order_id} returning id::text`;return {status,campaignId:campaign?.id};});if(result.campaignId)await database()`insert into campaign_status_history(campaign_id,from_status,to_status,note,changed_by) values (${result.campaignId},'PAYMENT_REVIEW',${result.status==="APPROVED"?"PENDING_REVIEW":"PENDING_PAYMENT"},${body.reason||"Pago revisado"},${actor.id})`;await persistAudit(actor,"ADVERTISING_PAYMENT_REVIEWED","ADVERTISING_PAYMENT",id,`${body.decision}: ${body.reason}`);return result;}catch(error){return adminError(error,reply);} });
  ...
  app.post("/v1/admin/commercial/campaigns/:id/action",async(request,reply)=>{try{const actor=requirePermission(request,"commercial:campaigns:manage"),id=z.string().uuid().parse((request.params as any).id),body=campaignActionSchema.parse(request.body);if(["REJECT","REQUEST_CORRECTION"].includes(body.action)&&body.note.length<5)return reply.code(400).send({error:"NOTE_REQUIRED"});const [campaign]=await database()`select starts_at,ends_at from affiliate_banners where id=${id}`;if(!campaign)throw new Error("CAMPAIGN_NOT_FOUND");let to:string;if(body.action==="APPROVE")to=new Date(String(campaign.starts_at))>new Date()?"SCHEDULED":"ACTIVE";else to={REJECT:"REJECTED",REQUEST_CORRECTION:"PENDING_REVIEW",PAUSE:"PAUSED",RESUME:new Date(String(campaign.starts_at))>new Date()?"SCHEDULED":"ACTIVE",CANCEL:"CANCELLED"}[body.action]!;await transitionCampaign(id,to,actor,body.note);if(body.action==="REQUEST_CORRECTION"){const [lead]=await database()`select o.lead_id::text as id from affiliate_banners b join advertising_orders o on o.id=b.order_id where b.id=${id}`;if(lead?.id)await createInvitation(String(lead.id),"ADMIN",actor.id,"CORRECTION",["campaign.image"]);}return {id,status:to};}catch(error){return adminError(error,reply);} });
