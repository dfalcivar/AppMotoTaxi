...
export async function registerCommercialRoutes(app: FastifyInstance) {
  ...
  app.post("/v1/public/advertising/chat/submit", async (request, reply) => { try {
    ...
    const result = await database().begin(async tx => {
      ...
      const amount = Number(plan.price ?? plan.monthly_price ?? 0), duration = Number(plan.duration_days ?? 30), start = new Date(body.campaign.startsAt), end = new Date(start.getTime() + duration * 86_400_000), placement = normalizeAdvertisingPlacement(plan.placement ?? plan.allowed_placements?.[0]), category = advertisingCategoryFromPlacements(plan.allowed_placements);
  ...
  app.post("/v1/public/advertising/invitations/:token/submit", async (request, reply) => { try {
    ...
    const result = await database().begin(async tx => {
      ...
      const amount = Number(plan.price ?? plan.monthly_price ?? 0), duration = Number(plan.duration_days ?? 30), start = new Date(body.campaign.startsAt), end = new Date(start.getTime() + duration * 86_400_000), campaignPlacement = normalizeAdvertisingPlacement(plan.placement ?? plan.allowed_placements?.[0]), category = advertisingCategoryFromPlacements(plan.allowed_placements);
  ...
  app.post("/v1/admin/commercial/campaigns/:id/renew",async(request,reply)=>{try{
    ...
    const result=await database().begin(async tx=>{
      ...
      if(!plan)throw new Error("PLAN_NOT_FOUND");
      const duration=Number(plan.duration_days??30),amount=Number(plan.price??plan.monthly_price??0),window=advertisingRenewalWindow(source.ends_at,duration),placement=normalizeAdvertisingPlacement(plan.placement??plan.allowed_placements?.[0]),category=advertisingCategoryFromPlacements(plan.allowed_placements);
