...
beforeAll(async()=>{pg=new PGlite();state.sql=sqlFor(pg);await pg.exec(`
 ...
 `);for(const file of ['101_costa_go_benefits.sql','102_referrals.sql','103_referral_rule_snapshots.sql'])await pg.exec(await readFile(new URL('../migrations/'+file,import.meta.url),'utf8'));
