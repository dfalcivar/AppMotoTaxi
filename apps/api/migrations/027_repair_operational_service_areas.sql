-- Reparación idempotente para instalaciones donde 025 se ejecutó sin un
-- administrador disponible y, por tanto, no llegó a publicar las zonas base.
-- Los campos de auditoría aceptan NULL para que la disponibilidad de las zonas
-- no dependa del orden de creación de la cuenta administrativa.
...
WITH source_geometries AS (
  ...
  SELECT area.id, 1, candidate.geometry,
    'OpenStreetMap contributors; límite comercial Piedra Fina A',
    'https://www.openstreetmap.org/copyright',
    'Versión inicial reparada: corte oriental Piedra Fina A.', area.created_by
  FROM service_areas area CROSS JOIN candidate
  WHERE area.code = 'ATACAMES_PROD' AND area.current_version_id IS NULL
  RETURNING id, service_area_id
)
UPDATE service_areas area SET current_version_id = published.id, updated_at = now()
FROM published WHERE area.id = published.service_area_id;
...
WITH published AS (
  ...
  SELECT area.id, 1,
    ...
    'Versión inicial reparada para pruebas controladas.', area.created_by
