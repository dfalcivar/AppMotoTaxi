...
La presentación de fotografías se genera con Sharp exclusivamente cuando `kind=PHOTO`: orientación EXIF, lienzo horizontal **1200 × 900 (4:3)**, WebP optimizado y sin metadatos EXIF públicos. La fotografía completa y sin correcciones de color forma el primer plano, centrada, con margen uniforme y sin ampliación artificial si el archivo es pequeño. El espacio restante se completa con una copia ampliada, atenuada y desenfocada de la misma fotografía; no hay franjas negras ni transparentes. Los documentos, perfiles, comprobantes y publicidades conservan sus flujos y tratamientos anteriores.

**Límite deliberado:** no se usa IA generativa, no se inventan partes y no se recorta el primer plano mediante una detección incierta. El encuadre seguro contiene la fotografía real completa para no quitar ruedas, techo, placa, accesorios o adhesivos. La posición `attention` de Sharp se utiliza únicamente para componer el fondo desenfocado; nunca decide qué parte del vehículo conservar. Una foto tomada desde muy lejos seguirá mostrando el entorno: conviene subir una fotografía cercana y bien encuadrada.

Prioridad resuelta por el endpoint autorizado existente: versión `display` válida de 1200 × 900 → normalización del original si la vista es legacy/falta → original si falla la recuperación → icono en UI si no hay imagen utilizable. Las miniaturas antiguas se regeneran al leerlas sin sobrescribir archivos históricos. Las nuevas fotografías se aceptan solamente si puede construirse su vista normalizada; el original exacto permanece inmutable. Caché local versionada y limitada a 16 imágenes/8 MB, con decodificación legacy serializada para evitar picos de memoria y duplicación simultánea de trabajo.
